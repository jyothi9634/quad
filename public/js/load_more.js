...
jQuery(document).ready(function() {
	...
	$('.article_navigation a').each(function(index) {
	...
	$(window).scroll(function(e){
		...
		//console.log($(window).scrollTop() + " -- "+ screen.height + "---" + "--" + $(window).height() +" --  " + $(document).height());
		//if($(window).scrollTop() + $(window).height() >= $(document).height() - 300) {
		if($(window).scrollTop() + screen.height  >= $(document).height() - 300) {
			//console.log("bottom of the page reached!");
			//console.log(pages);console.log(loaded);console.log(current);
			...
			$('#loader').fadeIn('slow', function() {
				$.get(pages[position], function(data) {
					$('#loader').fadeOut('slow', function() {
						$('#article_posts').append(data.html).fadeIn(999);
						current=position;
					});
					//$('.ajax_loaded').readmore();
				});
			});
		}
	}
