...
class PtlSellerListingComponent {
	 ...
	 * @return Response
	 */
	public static function PTLSellerList($statusId, $roleId, $serviceId) {
		...
		if(isset($_REQUEST['lead_name']) && ($_REQUEST['lead_name'] ==2)){
			...
			Session::put('leads', '1');
			$Query->leftjoin ( 'buyer_quote_selected_sellers as bqss', 'bqss.seller_id', '=', 'pspi.created_by' );
		}
		...
		foreach($sellerresults as $seller){
			$seller_post_items  = DB::table('ptl_seller_post_items')
				->join ( 'ptl_seller_posts', 'ptl_seller_posts.id', '=', 'ptl_seller_post_items.seller_post_id' )
				->where('ptl_seller_post_items.seller_post_id',$seller->id)
				->where ( 'ptl_seller_posts.lkp_ptl_post_type_id', 1 )
				->select('*')
				->get();
			foreach($seller_post_items as $seller_post_item){
		...
		$grid->row ( function ($row) {
			...
			$row->cells [1]->value = '<span class="lbl padding-8"></span>'.$frmdate;
			...
			$getpostitemids = DB::table('ptl_seller_post_items')
				->where('ptl_seller_post_items.seller_post_id','=',$spId)
				->select('ptl_seller_post_items.*')
				->get();
			...
			$Ptlseller_post_items  = DB::table('ptl_seller_post_items')
				->join('ptl_seller_posts','ptl_seller_posts.id','=','ptl_seller_post_items.seller_post_id')
				->where('ptl_seller_post_items.seller_post_id',$spId)
				->select('*')
				->get();
			...
			$seller_post_items  = DB::table('ptl_seller_post_items')
				->join ( 'ptl_seller_posts', 'ptl_seller_posts.id', '=', 'ptl_seller_post_items.seller_post_id' )
				->where('ptl_seller_post_items.seller_post_id',$spId)
				->select('ptl_seller_post_items.*','ptl_seller_posts.lkp_ptl_post_type_id','ptl_seller_post_items.from_location_id','ptl_seller_post_items.to_location_id')
				->get();
			...
			$docs_seller_ptl    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$spId);
			...
			if($privatepost==1){
				...
				$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(ROAD_PTL,$seller_post_items[0]->seller_post_id));
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)){
					for($k=0;$k<count($seller_post_items);$k++)
						...
						$total_count += count(SellerMatchingComponent::getMatchedResults(ROAD_PTL, $seller_post_items[$k]->id));
				}
				else
					$total_count =0;
			...
			if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)){
				for($k=0;$k<count($seller_post_items);$k++)
					//$total_count += PtlSellerListingComponent::enquiryCount(2,$seller_post_items[$k]->from_location_id,$seller_post_items[$k]->to_location_id,$seller_post_items[$k]->lkp_ptl_post_type_id );
				$lead_count += count(SellerMatchingComponent::getSellerLeads(ROAD_PTL, $seller_post_items[$k]->id));
			}else
				$lead_count =0;
			}
			...
			$row->cells [5]->value .='
							...
							<a ><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_ptl).'</span></a>
						</div>
					</div>
					<div class="pull-right text-right">
						<div class="info-links">';
			if($Ptlseller_post_items[0]->tracking != 0){
		...
		if(isset($_REQUEST['search']) && isset($_REQUEST['lkp_ptl_post_type_id']) && $_REQUEST['lkp_ptl_post_type_id']==1){
		...
		}elseif(isset($_REQUEST['search']) && isset($_REQUEST['lkp_ptl_post_type_id']) && $_REQUEST['lkp_ptl_post_type_id']==2){
	...
	public static function listPTLSellerPostItems($statusId, $roleId, $serviceId, $id){
		try{
			...
			$lkppoststatus  = DB::table('ptl_seller_posts')
				->where('ptl_seller_posts.id','=',$id)
				->select('ptl_seller_posts.lkp_ptl_post_type_id')
				->get();

			Session::put('lkppoststatus', $lkppoststatus[0]->lkp_ptl_post_type_id);
			$Query = DB::table ( 'ptl_seller_posts as sp' );
			$Query->leftjoin ( 'ptl_seller_post_items as spi', 'spi.seller_post_id', '=', 'sp.id' );
			...
			if(Session::get('leads') &&  Session::get('leads')==2){
				...
				$Query->leftjoin ( 'ptl_buyer_quote_sellers_quotes_prices as bqss', 'bqss.seller_id', '=', 'spi.created_by' );
			}
			...
			$grid->row ( function ($row) {
				...
				$row->cells [1]->value = '<span class="lbl padding-8"></span>' .CommonComponent::getZoneName($row->cells [1]->value);
				} else {
				$row->cells [1]->value = '<span class="lbl padding-8"></span>' . CommonComponent::getZonePin($row->cells [1]->value);
				}
				//$row->cells [1]->value = '<input type="checkbox" name="sellerpostcheck" id="sellerpostcheck" class="checkBoxClass gridcheckboxitems" value=' . $spId . '><span class="lbl padding-8"></span>' . $row->cells [1]->value;
				//$row->cells [5]->style('text-align:right');
				//$row->cells [5]->value = 'N/A';
				$row->cells [6]->style('width:100%');
				//View Count
				$countview = DB::table('ptl_seller_post_item_views')
					->where('ptl_seller_post_item_views.seller_post_item_id', '=', $spId)
					->select('ptl_seller_post_item_views.id', 'ptl_seller_post_item_views.view_counts')
					->get();
				if (!isset($countview[0]->view_counts))
					$countview = 0;
				...
				$seller_post_items = DB::table('ptl_seller_post_items')
					->join('ptl_seller_posts', 'ptl_seller_posts.id', '=', 'ptl_seller_post_items.seller_post_id')
					->where('ptl_seller_post_items.id', $spId)
					->select('ptl_seller_post_items.*', 'ptl_seller_posts.lkp_ptl_post_type_id','ptl_seller_posts.id as spid')
					->get();
				...
				//$row->cells [7]->attributes(array("class" => "col-md-12 col-sm-12 col-xs-12 pull-right padding-none below_each_row_grid count_block"));

				//Enquiries count
				$total_count = 0;
				if($seller_post_items[0]->is_private == 1){
					$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(ROAD_PTL,$seller_post_items[0]->spid));
				}else{
					if (isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)) {
						$total_count += count(SellerMatchingComponent::getMatchedResults(ROAD_PTL, $spId));
					}else
						$total_count =0;
				}
				...
				if($seller_post_items[0]->is_private == 1){
					...
					if (isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)) {
						...
						$lead_count += count(SellerMatchingComponent::getSellerLeads(ROAD_PTL, $spId));
					}else
						$lead_count =0;
				}
			...
			$row->cells [7]->value .='<div class="clearfix"></div>
								<div class="pull-left">
									<div class="info-links">
										...
										<a href="/sellerpostdetail/'.$spId.'?type=documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_ptl).'</span></a>
									</div>
								</div>
								<div class="pull-right text-right">
									<div class="info-links">';
                                	...
                                	$row->cells [7]->value .= '</a>';
										...
										$row->cells [7]->value .='<a href="/sellerpostdetail/'.$spId.'"><span class="views red"><i class="fa fa-eye" title="Views"></i>';
	...
	public static function listPTLSellertopNavPostItems($id){

		$seller_post_items  = DB::table('ptl_seller_posts')
			->where('ptl_seller_posts.id',$id)
			->select('ptl_seller_posts.id','ptl_seller_posts.transaction_id','ptl_seller_posts.kg_per_cft','ptl_seller_posts.pickup_charges','ptl_seller_posts.delivery_charges','ptl_seller_posts.oda_charges',
				'ptl_seller_posts.lkp_access_id','ptl_seller_posts.from_date',
				'ptl_seller_posts.to_date','ptl_seller_posts.lkp_payment_mode_id',
				'ptl_seller_posts.tracking','ptl_seller_posts.accept_payment_netbanking'
				,'ptl_seller_posts.accept_payment_credit','ptl_seller_posts.accept_payment_debit','ptl_seller_posts.credit_period','ptl_seller_posts.credit_period_units'
				,'ptl_seller_posts.accept_credit_netbanking','ptl_seller_posts.accept_credit_cheque'
				,'ptl_seller_posts.cancellation_charge_text','ptl_seller_posts.cancellation_charge_price'
				,'ptl_seller_posts.docket_charge_text','ptl_seller_posts.docket_charge_price'
				,'ptl_seller_posts.other_charge1_text','ptl_seller_posts.other_charge1_price'
				,'ptl_seller_posts.other_charge2_text','ptl_seller_posts.other_charge2_price'
				,'ptl_seller_posts.other_charge3_text','ptl_seller_posts.other_charge3_price', 'ptl_seller_posts.terms_conditions')
			->get();
		...
		if($seller_post_items[0]->lkp_access_id == 2 || $seller_post_items[0]->lkp_access_id == 3)
			$privatebuyers  = DB::table('ptl_seller_sellected_buyers')
			->leftjoin ( 'ptl_seller_posts', 'ptl_seller_posts.id', '=', 'ptl_seller_sellected_buyers.seller_post_id' )
			->leftjoin('users','users.id','=','ptl_seller_sellected_buyers.buyer_id')
			->where('ptl_seller_sellected_buyers.created_by',Auth::user()->id)
			->where('ptl_seller_sellected_buyers.seller_post_id',$id)
	...
	$postdetails .='<div>
			...
			if($seller_post_items[0]->kg_per_cft !='' && !empty($seller_post_items[0]->kg_per_cft)){
				$postdetails .='<div class="col-md-2 padding-left-none data-fld">
												<span class="data-head">Kg per CFT</span>
	...
	public static function listPTLSellerPostDetailsItems($id){
		Session::put('ptl_seller_post_item', $id);
		try{

			$countview = DB::table('ptl_seller_post_item_views')
				->where('ptl_seller_post_item_views.seller_post_item_id','=',$id)
				->select('ptl_seller_post_item_views.id','ptl_seller_post_item_views.view_counts')
				->get();
			if(!isset($countview[0]->view_counts))
				$countview = 0;
			...
			$seller_post = DB::table('ptl_seller_posts')
				->join('ptl_seller_post_items','ptl_seller_post_items.seller_post_id','=','ptl_seller_posts.id')
				->where('ptl_seller_post_items.id',$id)
				->select('ptl_seller_posts.*','ptl_seller_post_items.id')
				->get();
			$sellerpostid = DB::table('ptl_seller_posts')
							->join('ptl_seller_post_items','ptl_seller_post_items.seller_post_id','=','ptl_seller_posts.id')
							->where('ptl_seller_post_items.id',$id)
							->select('ptl_seller_posts.id')
							->get();
			...
			//count for seller documents
			$serviceId = Session::get('service_id');
			$docs_seller_ptl    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$sellerpostid[0]->id);
			...
			$matchedIds = array();
			if($seller_post_items[0]->is_private == 1){
				$matchedIds[] = CommonComponent::getPrivateBuyerMatchedResults(ROAD_PTL,$seller_post_items[0]->seller_post_id);
			}
			else{
				$buyer_quote_items_matched_data = SellerMatchingComponent::getMatchedResults(ROAD_PTL, $id);
			...
			if(!empty($matchedIds) && count($matchedIds) > 0){
				...
				$buyerquoteid    = DB::table('ptl_buyer_quotes')
					->leftjoin('ptl_buyer_quote_items','ptl_buyer_quote_items.buyer_quote_id','=','ptl_buyer_quotes.id')
					->leftjoin('users','users.id','=','ptl_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','ptl_buyer_quote_items.lkp_load_type_id')
					->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','ptl_buyer_quotes.from_location_id')
					->leftjoin('lkp_packaging_types','lkp_packaging_types.id','=','ptl_buyer_quote_items.lkp_packaging_type_id')
					->leftjoin('lkp_ict_weight_uom','lkp_ict_weight_uom.id','=','ptl_buyer_quote_items.lkp_ict_weight_uom_id')
					->leftjoin('lkp_cities','lkp_cities.id','=','ptl_buyer_quotes.from_location_id')
					->whereIn('ptl_buyer_quotes.id',$matchedIds)
					->select('ptl_buyer_quotes.transaction_id as transaction_no','ptl_buyer_quotes.id as ptlquoteid','ptl_buyer_quote_items.id','ptl_buyer_quote_items.created_by as buyer_id','users.username','ptl_buyer_quotes.dispatch_date',
						'ptl_buyer_quotes.delivery_date','ptl_buyer_quote_items.lkp_quote_price_type_id',
						'ptl_buyer_quotes.from_location_id','ptl_buyer_quotes.to_location_id',
						'lkp_cities.city_name','ptl_buyer_quotes.from_location_id',
						'ptl_buyer_quote_items.lkp_load_type_id','ptl_buyer_quotes.lkp_post_status_id',
						'ptl_buyer_quote_items.lkp_packaging_type_id',
						'ptl_buyer_quote_items.calculated_volume_weight','ptl_buyer_quotes.is_door_pickup','ptl_buyer_quotes.is_door_delivery',
						'ptl_buyer_quote_items.units','ptl_buyer_quote_items.lkp_ict_weight_uom_id',
						'ptl_buyer_quote_items.number_packages','lkp_load_types.load_type','lkp_ptl_pincodes.postoffice_name',
						'lkp_ptl_pincodes.pincode','lkp_packaging_types.packaging_type_name','lkp_ict_weight_uom.weight_type'
					)
					->groupBy('ptl_buyer_quotes.id')
					->get();
				
				for($i=0;$i<count($buyerquoteid);$i++){
					$buyersquotes	= DB::table('ptl_buyer_quote_sellers_quotes_prices')
					->where('ptl_buyer_quote_sellers_quotes_prices.buyer_quote_id',$buyerquoteid[$i]->ptlquoteid)
					->where('ptl_buyer_quote_sellers_quotes_prices.buyer_id',$buyerquoteid[$i]->buyer_id)
				...
				$buyerpublicquotedetails[]    = DB::table('ptl_buyer_quotes')
					->leftjoin('ptl_buyer_quote_items','ptl_buyer_quote_items.buyer_quote_id','=','ptl_buyer_quotes.id')
					->leftjoin('users','users.id','=','ptl_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','ptl_buyer_quote_items.lkp_load_type_id')
					->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','ptl_buyer_quotes.from_location_id')
					...
					->leftjoin('lkp_cities','lkp_cities.id','=','ptl_buyer_quotes.from_location_id')
					->whereIn('ptl_buyer_quotes.id',$matchedIds)
					->select('ptl_buyer_quotes.id as ptlquoteid','ptl_buyer_quote_items.id','ptl_buyer_quote_items.created_by as buyer_id','users.username','ptl_buyer_quotes.dispatch_date',
						'ptl_buyer_quotes.delivery_date','ptl_buyer_quote_items.lkp_quote_price_type_id',
						'ptl_buyer_quotes.from_location_id','ptl_buyer_quotes.to_location_id',
						'lkp_cities.city_name','ptl_buyer_quotes.from_location_id',
						'ptl_buyer_quote_items.lkp_load_type_id','ptl_buyer_quotes.lkp_post_status_id',
					...
					->groupBy('ptl_buyer_quote_items.id')
					->get();
			...
			if($seller_post_items[0]->is_private == 1){
				...
				$buyer_quote_items_leads_data = SellerMatchingComponent::getSellerLeads(ROAD_PTL, $id);
			...
			if(!empty($matchedLeadsIds) && count($matchedLeadsIds) > 0){
				...
				$buyerleadquoteid    = DB::table('ptl_buyer_quotes')
				->leftjoin('ptl_buyer_quote_items','ptl_buyer_quote_items.buyer_quote_id','=','ptl_buyer_quotes.id')
				->leftjoin('users','users.id','=','ptl_buyer_quotes.created_by')
				->leftjoin('lkp_load_types','lkp_load_types.id','=','ptl_buyer_quote_items.lkp_load_type_id')
				->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','ptl_buyer_quotes.from_location_id')
				...
				->leftjoin('lkp_ict_weight_uom','lkp_ict_weight_uom.id','=','ptl_buyer_quote_items.lkp_ict_weight_uom_id')
				->leftjoin('lkp_cities','lkp_cities.id','=','ptl_buyer_quotes.from_location_id')
				->whereIn('ptl_buyer_quotes.id',$matchedLeadsIds)
				->select('ptl_buyer_quotes.transaction_id as transaction_no','ptl_buyer_quotes.id as ptlquoteid','ptl_buyer_quote_items.id','ptl_buyer_quote_items.created_by as buyer_id','users.username','ptl_buyer_quotes.dispatch_date',
						'ptl_buyer_quotes.delivery_date','ptl_buyer_quote_items.lkp_quote_price_type_id',
						'ptl_buyer_quotes.from_location_id','ptl_buyer_quotes.to_location_id',
						'lkp_cities.city_name','ptl_buyer_quotes.from_location_id',
				...
				->groupBy('ptl_buyer_quotes.id')
				->orderBy('ptl_buyer_quotes.dispatch_date', 'asc')
				->get();
			
				
				for($i=0;$i<count($buyerleadquoteid);$i++){
					$buyersquotes	= DB::table('ptl_buyer_quote_sellers_quotes_prices')
					->where('ptl_buyer_quote_sellers_quotes_prices.buyer_quote_id',$buyerleadquoteid[$i]->ptlquoteid)
					->where('ptl_buyer_quote_sellers_quotes_prices.buyer_id',$buyerleadquoteid[$i]->buyer_id)
					->where('ptl_buyer_quote_sellers_quotes_prices.seller_id',Auth::user()->id)
					->select('ptl_buyer_quote_sellers_quotes_prices.*')
					->get();
				...
				$buyerleadquotedetails[]    = DB::table('ptl_buyer_quotes')
				->leftjoin('ptl_buyer_quote_items','ptl_buyer_quote_items.buyer_quote_id','=','ptl_buyer_quotes.id')
				->leftjoin('users','users.id','=','ptl_buyer_quotes.created_by')
				->leftjoin('lkp_load_types','lkp_load_types.id','=','ptl_buyer_quote_items.lkp_load_type_id')
				->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','ptl_buyer_quotes.from_location_id')
				...
				->groupBy('ptl_buyer_quote_items.id')
				->orderBy('ptl_buyer_quote_items.id', 'desc')
				->get();
		...
		if($seller_post[0]->lkp_post_status_id ==1){
			...
			if($seller_post_items[0]->is_private == 1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post[0]->lkp_ptl_post_type_id)) {
					...
					$total_count = count(SellerMatchingComponent::getMatchedResults(ROAD_PTL, $seller_post_items[0]->id));
				}else
					$total_count =0;
			}
		}
		...
		if($seller_post[0]->lkp_post_status_id ==1){
			...
			if($seller_post_items[0]->is_private == 1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post[0]->lkp_ptl_post_type_id)) {
					//$total_count = PtlSellerListingComponent::enquiryCount(2,$seller_post_items[0]->from_location_id,$seller_post_items[0]->to_location_id,$seller_post[0]->lkp_ptl_post_type_id );
					$lead_count = count(SellerMatchingComponent::getSellerLeads(ROAD_PTL, $seller_post_items[0]->id));
				}else
					$lead_count =0;
			}
		}
		...
		$post_details='<div class="col-md-12 col-sm-12 col-xs-12 padding-none details_block">
							...
							<h5>
								<div class="col-md-4 col-sm-5 col-xs-6 padding-none"><b>Lessthan Truck Load</b></div>
		...
		$post_details .='</p>
								...
								<p>'.$payment_type.'<span class="pull-right spot_transaction_details mobile-margin-top mobile-margin-bottom"><span class="show_details">+</span><span class="hide_details">-</span> Details</span>
								</p>
							</div>
							<div class="clearfix"></div>
		...
		if($seller_post[0]->pickup_charges !='')
			$post_details .= '<p>Pickup charges</p>';

		if($seller_post[0]->delivery_charges !='')
			$post_details .= '<p>Delivery charges</p>';

		if($seller_post[0]->oda_charges !='')
			$post_details .= '<p>ODA charges</p>';

		$post_details .= '<p>Documents</p>
								</div>
								<div class="col-md-8 col-sm-7 col-xs-6 padding-none">
									<p>'.$tracking.'</p>
									<p>';
		if($seller_post[0]->kg_per_cft==0)
			$post_details .= '0';
		else
			$post_details .= $seller_post[0]->kg_per_cft;
		$post_details .='</p>
									<p>';
		if($seller_post[0]->cancellation_charge_price!='')
			$post_details .= $seller_post[0]->cancellation_charge_price;
		$post_details .= '</p>
									<p>';
		if($seller_post[0]->docket_charge_price!='')
		...
		if($seller_post[0]->pickup_charges !=''){
			$post_details .= '<p>'.$seller_post[0]->pickup_charges.'</p>';
		}

		if($seller_post[0]->delivery_charges !=''){
			$post_details .= '<p>'.$seller_post[0]->delivery_charges.'</p>';
		}

		if($seller_post[0]->oda_charges !=''){
			$post_details .= '<p>'.$seller_post[0]->oda_charges.'</p>';
		}
		
		$post_details .= '<p><i class="fa fa-lg fa-file-text"></i> <sup class="">0</sup></p>
								</div>
							</div>
						</div>';
		...
		$gridtopnav = ' <a href="#" class="'.$color1.'"  data-showdiv="ftl-seller-messages" title="messages"><i class="fa fa-envelope-o"></i> Messages<span class="badge">'.$countMessages.'</span></a>
						...
						<a href="#"  class="'.$color4.'" data-showdiv="ftl-seller-documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_ptl).'</span></a>';
		...
		if($seller_post[0]->lkp_access_id == 2 || $seller_post[0]->lkp_access_id == 3)
			...
			->leftjoin('users','users.id','=','ptl_seller_sellected_buyers.buyer_id')
			->where('ptl_seller_sellected_buyers.created_by',Auth::user()->id)
			->where('ptl_seller_sellected_buyers.seller_post_id',$sellerpostid[0]->id)
			->select('users.username')
			->get();
		else
			$privatebuyers =array();
	...
	public static function getPTLSellerPostItemDetails($id,$data){
            $serviceId = Session::get('service_id');
		if($serviceId!=RELOCATION_DOMESTIC && $serviceId!=RELOCATION_OFFICE_MOVE && $serviceId!=RELOCATION_PET_MOVE && $serviceId!=RELOCATION_INTERNATIONAL && $serviceId!=RELOCATION_GLOBAL_MOBILITY){
                    $str_from = "search_ptl_buyer_from_id_".$id;
                    $from_id = $data[$str_from];
                    $str_to = "search_ptl_buyer_to_id_".$id;
                    $to_id = $data[$str_to];
                }else{
                $to_id='';
                }
		...
		try{
			...
			switch($serviceId){
                ...
                case AIR_DOMESTIC:
					...
					->where('airdom_seller_post_item_views.seller_post_item_id','=',$id)
                ...
                case AIR_INTERNATIONAL:
					...
					->where('airint_seller_post_item_views.seller_post_item_id','=',$id)
                ...
                case RELOCATION_DOMESTIC:
	                $countview = DB::table('relocation_seller_post_views')
					->where('relocation_seller_post_views.seller_post_item_id','=',$id)
					->select('relocation_seller_post_views.id','relocation_seller_post_views.view_counts')
					->get();
                    break;
                case RELOCATION_INTERNATIONAL:
	                $countview = DB::table('relocationint_seller_post_views')
					->where('relocationint_seller_post_views.seller_post_item_id','=',$id)
					->select('relocationint_seller_post_views.id','relocationint_seller_post_views.view_counts')
					->get();
                    break;  
                case RELOCATION_OFFICE_MOVE:
	                $countview = DB::table('relocationoffice_seller_post_views')
					->where('relocationoffice_seller_post_views.seller_post_id','=',$id)
					->select('relocationoffice_seller_post_views.id','relocationoffice_seller_post_views.view_counts')
					->get();
                    break; 
                case RELOCATION_PET_MOVE:
	                $countview = DB::table('relocationpet_seller_post_views')
					->where('relocationpet_seller_post_views.seller_post_item_id','=',$id)
					->select('relocationpet_seller_post_views.id','relocationpet_seller_post_views.view_counts')
					->get();
                    break; 
                case RELOCATION_GLOBAL_MOBILITY:
	                $countview = DB::table('relocationgm_seller_post_views')
					->where('relocationgm_seller_post_views.seller_post_id','=',$id)
					->select('relocationgm_seller_post_views.id','relocationgm_seller_post_views.view_counts')
					->get();
                    break;
            }
            ...
            switch($serviceId){
                case ROAD_PTL:
	               $seller_post = DB::table('ptl_seller_posts as psp')
                            ->leftjoin('ptl_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'ptl_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                    DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays") )
                            ->get();
                    break;
                case RAIL:
	                $seller_post = DB::table('rail_seller_posts as psp')
                            ->leftjoin('rail_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'rail_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                   DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays")  )
                            ->get();
                    break;
                case AIR_DOMESTIC:
	                $seller_post = DB::table('airdom_seller_posts as psp')
                            ->leftjoin('airdom_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'airdom_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                   DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays")  )
                            ->get();
                    break;
                case AIR_INTERNATIONAL:
                	$seller_post = DB::table('airint_seller_posts as psp')
                            ->leftjoin('airint_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'airint_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                 DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays") )
                            ->get();
                    break;
                case OCEAN:
	                $seller_post = DB::table('ocean_seller_posts as psp')
                            ->leftjoin('ocean_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'ocean_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                   DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays") )
                            ->get();
                    break; 
                case COURIER:
	                $seller_post = DB::table('courier_seller_posts as psp')
                            ->leftjoin('courier_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'pspi.lkp_post_status_id')
                            ->leftjoin ( 'courier_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_item_id', '=', 'pspi.id' )
                            ->where('pspi.id',$id)
                            ->select('psp.*','pspi.*','users.username', 'lps.post_status',
                                   DB::raw("(case when `pbqsqp`.`final_transit_days` != 0 then pbqsqp.final_transit_days  when `pbqsqp`.`initial_transit_days` != 0 then pbqsqp.initial_transit_days when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays") )
                            ->get();
                    break;   
                case RELOCATION_DOMESTIC:
	                $seller_post = DB::table('relocation_seller_posts as psp')
                            ->leftjoin('relocation_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocation_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'pspi.id' )
                            ->where('psp.id',$id)
                            ->select('pspi.*','psp.*','users.username', 'lps.post_status',
                                    DB::raw("(case when `pbqsqp`.`transit_days` != 0 then pbqsqp.transit_days   when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays"))
                            ->get();
                    break;   
                case RELOCATION_INTERNATIONAL:
                	if(Session::get('searchMod.service_type_buyer') == INTERNATIONAL_TYPE_AIR){
                		 $seller_post = DB::table('relocationint_seller_posts as psp')
                            //->leftjoin('relocationint_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocationint_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'psp.id' )
                            ->where('psp.id',$id)
                            ->select('psp.*','users.username', 'lps.post_status',
                                    DB::raw("(case when `pbqsqp`.`transit_days` != 0 then pbqsqp.transit_days   when 'pbqsqp.id'=0 then psp.transitdays end) as transitdays"))
                            ->get();
                	}else if(Session::get('searchMod.service_type_buyer') == INTERNATIONAL_TYPE_OCEAN){
						$seller_post = DB::table('relocationint_seller_posts as psp')
                            ->leftjoin('relocationint_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocationint_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'pspi.id' )
                            ->where('psp.id',$id)
                            ->select('pspi.*','psp.*','users.username', 'lps.post_status',
                                    DB::raw("(case when `pbqsqp`.`transit_days` != 0 then pbqsqp.transit_days   when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays"))
                            ->get();
                    ...
                    break;        
                case RELOCATION_OFFICE_MOVE:
	                $seller_post = DB::table('relocationoffice_seller_posts as psp')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocationoffice_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'psp.id' )
                            ->where('psp.id',$id)
                            ->select('psp.*','users.username', 'lps.post_status')
                            ->get();
                    break;     
                case RELOCATION_PET_MOVE:
	                $seller_post = DB::table('relocationpet_seller_posts as psp')
                            ->leftjoin('relocationpet_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocationpet_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'pspi.id' )
                            ->where('psp.id',$id)
                            ->select('psp.*','psp.id as postid','pspi.*','users.username', 'lps.post_status',
                                    DB::raw("(case when `pbqsqp`.`transit_days` != 0 then pbqsqp.transit_days   when 'pbqsqp.id'=0 then pspi.transitdays end) as transitdays"))
                            ->get();
                    break;   
                case RELOCATION_GLOBAL_MOBILITY:
	                $seller_post = DB::table('relocationgm_seller_posts as psp')
                            //->leftjoin('relocation_seller_post_items as pspi','pspi.seller_post_id','=','psp.id')
                            ->leftjoin('users','users.id','=','psp.seller_id')
                            ->leftjoin('lkp_post_statuses as lps', 'lps.id', '=', 'psp.lkp_post_status_id')
                            ->leftjoin ( 'relocationgm_buyer_quote_sellers_quotes_prices as pbqsqp', 'pbqsqp.seller_post_id', '=', 'psp.id' )
                            ->where('psp.id',$id)
                            ->select('psp.*','users.username', 'lps.post_status')
                            ->get();
                    break;   
            ...
            if(!empty($seller_post)) {
                if($serviceId == RELOCATION_DOMESTIC || $serviceId == RELOCATION_PET_MOVE || $serviceId == RELOCATION_INTERNATIONAL){
                    $fromLocation = Session::get('searchMod.from_location_buyer');
                    $toLocation = Session::get('searchMod.to_location_buyer');
                    ...
                    $fromLocation = CommonComponent::getPinName($from_id);
                    if($serviceId == COURIER){
                            if($seller_post[0]->lkp_courier_delivery_type_id == IS_INTERNATIONAL){
                                    $toLocation = CommonComponent::getCountry($to_id);
                            }else{
                                    $toLocation = CommonComponent::getPinName($to_id);
                            }
                    }else{
                            $toLocation = CommonComponent::getPinName($to_id);
                    }           
                }
                $deliveryDate = CommonComponent::checkAndGetDate($seller_post[0]->to_date);
                $dispatchDate = CommonComponent::checkAndGetDate($seller_post[0]->from_date);
            } else {
                $fromLocation = '';
                $toLocation = '';
                $deliveryDate = '';
            ...
            if($serviceId == COURIER){
            	$kg_per_cft = $seller_post[0]->conversion_factor;
            }elseif($serviceId == RELOCATION_DOMESTIC || $serviceId == RELOCATION_PET_MOVE || $serviceId == RELOCATION_OFFICE_MOVE || $serviceId == RELOCATION_INTERNATIONAL || $serviceId == RELOCATION_GLOBAL_MOBILITY){
            	$kg_per_cft = 0;
            }else{
            	$kg_per_cft = $seller_post[0]->kg_per_cft;
            }
        ...
        /**
	 * LTL Buyer Private Posts List Page - Grid and filters
	 ...
	 */
	public static function listLTLBuyerPrivatePosts($statusId, $serviceId, $roleId,$type) {
		...
		// query to retrieve buyer posts list and bind it to the grid
                ...
                $Query->leftjoin ( 'ptl_buyer_quote_selected_sellers as bqss', 'bqss.buyer_quote_id', '=', 'ptlbq.id' );
		...
		foreach ( $postResults as $post ) {
			//$buyer_quotes = DB::table ( 'ptl_buyer_quote_items' )->where ( 'buyer_quote_id', $post->id )->select ( 'ptl_buyer_quote_items.*')->get ();
		...
		$grid->row ( function ($row) {
			...
			$buyerdetailsvalue = DB::table ( 'ptl_buyer_quote_items' )->where ( 'ptl_buyer_quote_items.id', '=', $buyer_quote_id )->select ( 'ptl_buyer_quote_items.*' )->get ();
			...
			$locationids = DB::table('ptl_buyer_quotes')
			->leftjoin('ptl_buyer_quote_items','ptl_buyer_quote_items.buyer_quote_id','=','ptl_buyer_quotes.id')
			->where('ptl_buyer_quote_items.id','=',$buyer_quote_id)
			->select('ptl_buyer_quotes.from_location_id','ptl_buyer_quotes.to_location_id')
			->first();
			...
			$row->cells [5]->value = $buyer_post_status;
							...
							$quoteid = DB::table('ptl_buyer_quote_items')
							->where('ptl_buyer_quote_items.id','=',$buyer_quote_id)
							->select('ptl_buyer_quote_items.buyer_quote_id')
							->get();
							
							$quoteitems = DB::table('ptl_buyer_quote_items')
							->join('lkp_load_types','lkp_load_types.id','=','ptl_buyer_quote_items.lkp_load_type_id')
							->join('lkp_packaging_types','lkp_packaging_types.id','=','ptl_buyer_quote_items.lkp_packaging_type_id')
							->where('ptl_buyer_quote_items.buyer_quote_id','=',$quoteid[0]->buyer_quote_id)
							->select('ptl_buyer_quote_items.*','lkp_load_types.load_type','lkp_packaging_types.packaging_type_name')
							->get();
							if(Session::get('service_id') == ROAD_PTL){
								$getInitialQuotePrice = CommonComponent::getPTLQuotePriceForSearch($buyer_id,$quoteid[0]->buyer_quote_id,Auth::user()->id,'initial_quote_price','ptl_buyer_quote_sellers_quotes_prices');
								$getCounterQuotePrice = CommonComponent::getPTLQuotePriceForSearch($buyer_id,$quoteid[0]->buyer_quote_id,Auth::user()->id,'counter_quote_price','ptl_buyer_quote_sellers_quotes_prices');
								$getFinalQuotePrice   = CommonComponent::getPTLQuotePriceForSearch($buyer_id,$quoteid[0]->buyer_quote_id,Auth::user()->id,'final_quote_price','ptl_buyer_quote_sellers_quotes_prices');
							}
							...
							$row->cells [5]->value .= '
							<div class="pull-right detailsslide1">
							...
							if(isset($getCounterQuotePrice[0]->counter_rate_per_kg) &&
									$getCounterQuotePrice[0]->counter_rate_per_kg !=''
									&& isset($getFinalQuotePrice[0]->final_kg_per_cft) && $getFinalQuotePrice[0]->final_kg_per_cft!=''){
								$row->cells [5]->value .= '<div class="pull-right detailsslide1">
							...
							<div class="col-md-12 padding-none">
								...
								<input type="text" name="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'" 
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'" 
                                    id="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'"
									placeholder="Rate per Kg *" class="ptl_initial_rate_per_kg form-control  form-control1" ></div>';
								...
								<div class="col-md-3 form-control-fld padding-left-none form-control-fld padding-top">
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_initial_conversion form-control  form-control1" >';
								...
								$row->cells [5]->value .= '<div class="clearfix"></div><div class="col-md-3 form-control-fld padding-none margin-top-none">
								...
								$row->cells [5]->value .='<div class="hide-final">';
									...
									placeholder="Rate per Kg *" class="ptl_final_rate_per_kg form-control form-control1 numberVal " ></div>';
								...
								elseif(isset($getFinalQuotePrice[0]->final_kg_per_cft) && $getFinalQuotePrice[0]->final_kg_per_cft=='' && $getInitialQuotePrice[0]->initial_kg_per_cft!=''  &&  $getCounterQuotePrice[0]->counter_rate_per_kg!='')
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_final_conversion form-control form-control1 numberVal " ></div>
								...
								if(isset($getInitialQuotePrice[0]->initial_quote_price) && $getInitialQuotePrice[0]->initial_quote_price=='0.0000'){
                                    ...
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'" 
                                    id="initial_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'" 
                                    placeholder="ODA Rs *" class="ptl_initial_oda form-control form-control1 numberVal ">';
									}else{
										$row->cells [5]->value .= '<input type="text"
								...
								elseif(isset($getCounterQuotePrice[0]->counter_quote_price) && $getCounterQuotePrice[0]->counter_quote_price !='0.0000' && isset($getFinalQuotePrice[0]->final_quote_price) && $getFinalQuotePrice[0]->final_quote_price=='0.0000'){
									if($odacheck == 1){
	                                    ...
	                                    placeholder="ODA Rs *" class="ptl_final_oda form-control form-control1 numberVal ">';
									}else{
										$row->cells [5]->value .= '
									<input type="text"
	...
	public static function listPTLBuyerMarketLeads($statusId, $roleId, $serviceId, $id){
		try{
			//Filters values to populate in the page
			$from_locationspincode = array(""=>"From Location");
			$to_locationspincode = array(""=>"To Location");
