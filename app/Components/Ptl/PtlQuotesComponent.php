...
class PtlQuotesComponent {
	 ...
	 * @return Response
	 */
	public static function PTLSellerQuoteSubmit($request) {
		try{
			...
			if(isset($sellerInput['buyer_buyerquote_id']) && !empty($sellerInput['buyer_buyerquote_id'])) {
				...
				$getSellerpostdetails  = DB::table('ptl_seller_post_items')
				->leftjoin('ptl_seller_posts','ptl_seller_posts.id','=','ptl_seller_post_items.seller_post_id')
				->where('ptl_seller_post_items.id','=',Session::get('seller_post_item'))
				->where('ptl_seller_post_items.created_by','=',Auth::user()->id)
				->select('ptl_seller_posts.*','ptl_seller_post_items.*')
				->get();
				
			
				$getBuyerpostdetails  = DB::table('ptl_buyer_quotes')
				->where('ptl_buyer_quotes.id','=',$buyerQuoteItemId)
				->where('ptl_buyer_quotes.created_by','=',$buyerId)
				->select('ptl_buyer_quotes.*')
				->get();
				...
				$createsellerpost = new PtlSellerPost();
				$createsellerpost->lkp_service_id = 2;
				...
				$createsellerpostitem = new PtlSellerPostItem();
				...
				$sellerselectedbuyer = new PtlSellerSellectedBuyer();
				...
				//CommonComponent::auditLog($sellerselectedbuyer->id,'ptl_seller_selected_buyers');
				...
				$getbqsqp = DB::table('ptl_buyer_quote_sellers_quotes_prices as bqsp')
                                ...
                                if(count($getbqsqp)>0){
                                        ...
                                        $updateinitial = DB::table('ptl_buyer_quote_sellers_quotes_prices')
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$createdIp = $_SERVER['REMOTE_ADDR'];
				$sellerinitialquote = new PtlBuyerQuoteSellersQuotesPrice();
				...
				CommonComponent::auditLog($sellerinitialquote->id,'ptl_buyer_quote_sellers_quotes_prices');
	 ...
	 * @return Response
	 */
	public static function PTLSellerFinalQuoteSubmit($request) {
		try{
			$formvalues = urldecode($_REQUEST['formvalues']);
			$formfields = explode("&", $formvalues);
			$hiddenfields = array();
			...
			CommonComponent::auditLog($buyerId,'ptl_buyer_quote_sellers_quotes_prices');
	 ...
	 * @return Response
	 */
	public static function PTLsellerSearchQuoteSubmit($request) {
		try{
			$formvalues = urldecode($_REQUEST['formvalues']);
			$formfields = explode("&", $formvalues);
			$hiddenfields = array();
			...
			$getBuyerpostdetails  = DB::table('ptl_buyer_quote_items')
			->leftjoin('ptl_buyer_quotes', 'ptl_buyer_quotes.id', '=', 'ptl_buyer_quote_items.buyer_quote_id')
			->where('ptl_buyer_quote_items.id','=',$buyerQuoteItemId)
			->where('ptl_buyer_quotes.created_by','=',$buyerId)
			->select('ptl_buyer_quotes.*')
			->get();
			...
			$sellerselectedbuyer->created_ip = $createdIp;
			$sellerselectedbuyer->save();
			//CommonComponent::auditLog($sellerselectedbuyer->id,'ptl_seller_sellected_buyers');
			...
			$initial_cretaed = date ( 'Y-m-d H:i:s' );
			$buyerinitial = new PtlBuyerQuoteSellersQuotesPrice();
			...
			//CommonComponent::auditLog($buyerinitial->id,'ptl_buyer_quote_sellers_quotes_prices');
			$seller_initial_quote_email = DB::table('users')->where('id', $buyerId)->get();
			$seller_initial_quote_email[0]->sellername = Auth::User()->username;
	 ...
	 * @return Response
	 */
	public static function PTLsellerCounterAcceptance($request) {
		try{
			...
			->where('ptl_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
			->where('ptl_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$buyerQuoteItemId)
			->where('ptl_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
