...
class FtlQuotesComponent {
	 ...
	 * @return Response
	 */
	public static function FTLSellerQuoteSubmit($request) {
		try{
			...
			if(isset($sellerInput['buyer_buyerquote_id']) && !empty($sellerInput['buyer_buyerquote_id'])
				...
				$getBuyerpostdetails  = DB::table('buyer_quote_items')
					->leftjoin('buyer_quotes', 'buyer_quotes.id', '=', 'buyer_quote_items.buyer_quote_id')
					->where('buyer_quote_items.id','=',$buyerQuoteItemId)
					->where('buyer_quote_items.created_by','=',$buyerId)
					->select('buyer_quote_items.*','buyer_quotes.transaction_id')
					->get();
					
				$getSellerpostdetails  = DB::table('seller_post_items')
					->leftjoin('seller_posts','seller_posts.id','=','seller_post_items.seller_post_id')
					->where('seller_post_items.id','=',Session::get('seller_post_item'))
					->where('seller_post_items.created_by','=',Auth::user()->id)
					->select('seller_posts.*','seller_post_items.*')
					->get();
				...
				$createsellerpost = new SellerPost();
				$createsellerpost->lkp_service_id = ROAD_FTL;
				...
				$createsellerpostitem = new SellerPostItem();
				...
				$getbqsqp = DB::table('buyer_quote_sellers_quotes_prices')
                                ...
                                ->select('buyer_quote_sellers_quotes_prices.id')
                                ->get();
                                if(count($getbqsqp)>0){
                                        $initial_cretaed = date ( 'Y-m-d H:i:s' );

                                        DB::table('buyer_quote_sellers_quotes_prices')
                                        ->where('buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
                                        ->where('buyer_quote_sellers_quotes_prices.buyer_quote_item_id','=',$buyerQuoteItemId)
                                        ->where('buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
                                        ->update(array('initial_quote_price' =>$_POST['initial_quote'],
                                                                'initial_transit_days' => $sellerInput['initial_transit'],'seller_post_item_id'=>$createsellerpostitem->id,
	 ...
	 * @return Response
	 */
	public static function FTLSellerFinalQuoteSubmit($request) {
		try{
	 ...
	 * @return Response
	 */
	public static function FTLSellerAcceptQuoteSubmit($request) {
		try{
	 ...
	 * @return Response
	 */
	public static function FTLSellerAcceptanceCounterOffer($request) {
		try{
			...
			if(!empty($sellerInput['buyer_buyerquote_id'])) {
				...
				->select('buyer_quote_sellers_quotes_prices.firm_price','buyer_quote_sellers_quotes_prices.counter_transit_days')
				->get();
	 ...
	 * @return Response
	 */
	public static function FTLsellerSearchQuoteSubmit($request) {
		try{
	 ...
	 * @return Response
	 */
	public static function FTLsellerCounterAcceptance($request) {
		try{
			...
			->where('buyer_quote_sellers_quotes_prices.buyer_quote_item_id','=',$buyerQuoteItemId)
			->where('buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
			->where('buyer_quote_sellers_quotes_prices.seller_post_item_id','!=',0)
			->select('buyer_quote_sellers_quotes_prices.counter_quote_price','buyer_quote_sellers_quotes_prices.counter_transit_days',
					'buyer_quote_sellers_quotes_prices.firm_price')
					->get();
			if(count($getcounter)>0){
				if(isset($getcounter[0]->firm_price) && $getcounter[0]->firm_price==0){
	 ...
	 * @return Response
	 */
	public static function FTLsellerSearchAcceptance($reques){
		try{
			...
			if($_POST['search']==1){
				...
				$final_cretaed = date ( 'Y-m-d H:i:s' );
				$createdIp = $_SERVER['REMOTE_ADDR'];
				$buyerfinal = new BuyerQuoteSellersQuotesPrices();
				...
				$buyerfinal->save();
				CommonComponent::auditLog($buyerfinal->id,'buyer_quote_sellers_quotes_prices');
