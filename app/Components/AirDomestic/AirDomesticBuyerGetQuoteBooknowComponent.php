...
class AirDomesticBuyerGetQuoteBooknowComponent {
    ...
    */
	public static function updateBuyerQuoteDetailsViews($buyerQuoteId) {
		try {
            ...
            $buyerCounterDetails  = DB::table('airdom_buyer_quote_items as bqi')
            ...
            if(!empty($buyerCounterDetails) && $buyerCounterDetails[0]->created_by != Auth::user()->id) {
                $viewCount = DB::table ( 'airdom_buyer_quote_item_views as bqiv' )
                ...
                if (count($viewCount) == 0) {
                    ...
                    DB::table ( 'airdom_buyer_quote_item_views as bqiv' )
                    			  ->where ( 'bqiv.user_id', '=', Auth::user ()->id )
                                  ->where ( 'bqiv.buyer_quote_item_id', '=', $buyerQuoteId )->update(array(
                                  'bqiv.view_counts' => $countview,
                                  'bqiv.updated_at' => $createdAt) );
                }
            } else {
				$countview = DB::table('airdom_buyer_quote_item_views as bqiv')
