...
class AirDomesticSellerListingComponent {
	 ...
	 * @return Response
	 */
	public static function AirDomesticSellerList($statusId, $roleId, $serviceId) {
		...
		if(isset($_REQUEST['lead_name']) && ($_REQUEST['lead_name'] ==2)){
			...
			Session::put('leads', '1');
			$Query->leftjoin ( 'airdom_buyer_quote_selected_sellers as bqss', 'bqss.seller_id', '=', 'pspi.created_by' );
		}
		...
		foreach($sellerresults as $seller){
			...
			->join ( 'airdom_seller_posts', 'airdom_seller_posts.id', '=', 'airdom_seller_post_items.seller_post_id' )
			->where('airdom_seller_post_items.seller_post_id',$seller->id)
			->where ( 'airdom_seller_posts.lkp_ptl_post_type_id', 1 )
			->select('*')
			->get();
			foreach($seller_post_items as $seller_post_item){
		...
		$grid->row ( function ($row) {
			...
			->where('airdom_seller_post_items.seller_post_id','=',$spId)
			->select('airdom_seller_post_items.*')
				->get();
			...
			$docs_seller_domestic   =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$spId);
			
			
			$Ptlseller_post_items  = DB::table('airdom_seller_post_items')
			->join('airdom_seller_posts','airdom_seller_posts.id','=','airdom_seller_post_items.seller_post_id')
			->where('airdom_seller_post_items.seller_post_id',$spId)
			->select('*')
			->get();
			...
			->join ( 'airdom_seller_posts', 'airdom_seller_posts.id', '=', 'airdom_seller_post_items.seller_post_id' )
			->where('airdom_seller_post_items.seller_post_id',$spId)
			->select('airdom_seller_post_items.*','airdom_seller_posts.lkp_ptl_post_type_id','airdom_seller_post_items.from_location_id','airdom_seller_post_items.to_location_id')
			->get();
			...
			if($privatepost==1){
				...
				$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(AIR_DOMESTIC,$seller_post_items[0]->seller_post_id));
				...
				for($k=0;$k<count($seller_post_items);$k++)
					$total_count += count(SellerMatchingComponent::getMatchedResults(AIR_DOMESTIC, $seller_post_items[$k]->id));
				}
				else 
					$total_count =0;
			}
			...
			if($privatepost==1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)){
					for($k=0;$k<count($seller_post_items);$k++)	
					$lead_count += count(SellerMatchingComponent::getSellerLeads(AIR_DOMESTIC, $seller_post_items[$k]->id));
				}else
					$lead_count =0;
				}
			...
			$row->cells [5]->value .='
							...
							<a><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_domestic).'</span></a>
						</div>
					</div>
	...
	public static function listAirdomSellerPostItems($statusId, $roleId, $serviceId, $id){
		try{
			...
			$lkppoststatus  = DB::table('airdom_seller_posts')
			->where('airdom_seller_posts.id','=',$id)
			->select('airdom_seller_posts.lkp_ptl_post_type_id')
			->get();
			
			Session::put('lkppoststatus', $lkppoststatus[0]->lkp_ptl_post_type_id);
			$Query = DB::table ( 'airdom_seller_posts as sp' );
			$Query->leftjoin ( 'airdom_seller_post_items as spi', 'spi.seller_post_id', '=', 'sp.id' );
			...
			if(Session::get('leads') &&  Session::get('leads')==2){
				...
				$Query->leftjoin ( 'airdom_buyer_quote_sellers_quotes_prices as bqss', 'bqss.seller_id', '=', 'spi.created_by' );
			}
			...
			$grid->add ( 'id', 'ID', true )->style ( "display:none" );
			$grid->add ( 'from_location_id', 'From Location', 'from_location_id' )->attributes(array("class" => "col-md-2 col-sm-2 col-xs-5 padding-none"));
			...
			$grid->row ( function ($row) {
				...
				//View Count
				$countview = DB::table('airdom_seller_post_item_views')
				->where('airdom_seller_post_item_views.seller_post_item_id','=',$spId)
				->select('airdom_seller_post_item_views.id','airdom_seller_post_item_views.view_counts')
				->get();
				if(!isset($countview[0]->view_counts))
					$countview = 0;
				...
				$seller_post_items  = DB::table('airdom_seller_post_items')
				->join ( 'airdom_seller_posts', 'airdom_seller_posts.id', '=', 'airdom_seller_post_items.seller_post_id' )
				->where('airdom_seller_post_items.id',$spId)
				->select('airdom_seller_post_items.*','airdom_seller_posts.lkp_ptl_post_type_id','airdom_seller_posts.id as spid')
				->get();
				
				//count for seller documents
				$serviceId = Session::get('service_id');
				$docs_seller_domestic    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$seller_post_items[0]->spid);
				...
				if($seller_post_items[0]->is_private == 1){
					$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(AIR_DOMESTIC,$seller_post_items[0]->spid));
				}else{
					if (isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)) {
						...
						$total_count += count(SellerMatchingComponent::getMatchedResults(AIR_DOMESTIC, $spId));
					}else
						$total_count =0;
				}
				//Leads Count
				...
				if($seller_post_items[0]->is_private == 1){
					...
					if (isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)) {
						$lead_count += count(SellerMatchingComponent::getSellerLeads(AIR_DOMESTIC, $spId));
					}else
						$lead_count =0;
				}
				$msg_count  =    MessagesComponent::listMessages(null,POSTENQURYMESSAGETYPE,null,$spId);
				$row->cells [7]->value .='<div class="clearfix"></div>
								<div class="pull-left">
									<div class="info-links">
										...
										<a href="/sellerpostdetail/'.$spId.'?type=documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_domestic).'</span></a>
									</div>
								</div>
								<div class="pull-right text-right">
									<div class="info-links">';
	...
	public static function listAirdomSellertopNavPostItems($id){
		
		$seller_post_items  = DB::table('airdom_seller_posts')
							->where('airdom_seller_posts.id',$id)
							->select('airdom_seller_posts.id','airdom_seller_posts.transaction_id','airdom_seller_posts.kg_per_cft',
									 'airdom_seller_posts.lkp_access_id','airdom_seller_posts.from_date','airdom_seller_posts.pickup_charges',
									 'airdom_seller_posts.to_date','airdom_seller_posts.lkp_payment_mode_id','airdom_seller_posts.delivery_charges',
									 'airdom_seller_posts.tracking','airdom_seller_posts.accept_payment_netbanking','airdom_seller_posts.oda_charges'
									 ,'airdom_seller_posts.accept_payment_credit','airdom_seller_posts.accept_payment_debit'
									 ,'airdom_seller_posts.accept_credit_netbanking','airdom_seller_posts.accept_credit_cheque','airdom_seller_posts.credit_period','airdom_seller_posts.credit_period_units'
									 ,'airdom_seller_posts.cancellation_charge_text','airdom_seller_posts.cancellation_charge_price'
									 ,'airdom_seller_posts.docket_charge_text','airdom_seller_posts.docket_charge_price','airdom_seller_posts.terms_conditions'
									 ,'airdom_seller_posts.other_charge1_text','airdom_seller_posts.other_charge1_price'
									 ,'airdom_seller_posts.other_charge2_text','airdom_seller_posts.other_charge2_price'
									 ,'airdom_seller_posts.other_charge3_text','airdom_seller_posts.other_charge3_price')
							->get();
		...
		if($seller_post_items[0]->lkp_access_id == 2 || $seller_post_items[0]->lkp_access_id == 3)
			$privatebuyers  = DB::table('airdom_seller_sellected_buyers')
			->leftjoin ( 'airdom_seller_posts', 'airdom_seller_posts.id', '=', 'airdom_seller_sellected_buyers.seller_post_id' )
			->leftjoin('users','users.id','=','airdom_seller_sellected_buyers.buyer_id')
			->where('airdom_seller_sellected_buyers.created_by',Auth::user()->id)
			->where('airdom_seller_sellected_buyers.seller_post_id',$id)
	...
	$postdetails .='<div>
		...
		if($seller_post_items[0]->kg_per_cft !='' && !empty($seller_post_items[0]->kg_per_cft)){
			$postdetails .='<div class="col-md-2 padding-left-none data-fld">
									...
									<span class="data-value">'.$seller_post_items[0]->kg_per_cft.'</span>
								</div>';
		}
	...
	public static function listAirdomSellerPostDetailsItems($id){
		Session::put('airdom_seller_post_item', $id);
		try{
			...
			$seller_post = DB::table('airdom_seller_posts')
			->join('airdom_seller_post_items','airdom_seller_post_items.seller_post_id','=','airdom_seller_posts.id')
			->where('airdom_seller_post_items.id',$id)
			->select('airdom_seller_posts.*','airdom_seller_post_items.id')
			->get();
			$sellerpostid = DB::table('airdom_seller_posts')
			->join('airdom_seller_post_items','airdom_seller_post_items.seller_post_id','=','airdom_seller_posts.id')
			->where('airdom_seller_post_items.id',$id)
			->select('airdom_seller_posts.id')
			->get();
		...
		$matchedIds = array();
		if($seller_post_items[0]->is_private == 1){
			$matchedIds[] = CommonComponent::getPrivateBuyerMatchedResults(AIR_DOMESTIC,$seller_post_items[0]->seller_post_id);
		}
		else{
			$buyer_quote_items_matched_data = SellerMatchingComponent::getMatchedResults(AIR_DOMESTIC, $id);
		...
		if(!empty($matchedIds) && count($matchedIds) > 0){
			...
			$buyerquoteid    = DB::table('airdom_buyer_quotes')
			->leftjoin('airdom_buyer_quote_items','airdom_buyer_quote_items.buyer_quote_id','=','airdom_buyer_quotes.id')
			->leftjoin('users','users.id','=','airdom_buyer_quotes.created_by')
			->leftjoin('lkp_load_types','lkp_load_types.id','=','airdom_buyer_quote_items.lkp_load_type_id')
			->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airdom_buyer_quotes.from_location_id')
			->leftjoin('lkp_packaging_types','lkp_packaging_types.id','=','airdom_buyer_quote_items.lkp_packaging_type_id')
			->leftjoin('lkp_ict_weight_uom','lkp_ict_weight_uom.id','=','airdom_buyer_quote_items.lkp_ict_weight_uom_id')
			->leftjoin('lkp_cities','lkp_cities.id','=','airdom_buyer_quotes.from_location_id')
			->whereIn('airdom_buyer_quotes.id',$matchedIds)
			->select('airdom_buyer_quotes.transaction_id as transaction_no','airdom_buyer_quotes.id as ptlquoteid','airdom_buyer_quote_items.id','airdom_buyer_quote_items.created_by as buyer_id','users.username','airdom_buyer_quotes.dispatch_date',
					'airdom_buyer_quotes.delivery_date','airdom_buyer_quote_items.lkp_quote_price_type_id',
					'airdom_buyer_quotes.from_location_id','airdom_buyer_quotes.to_location_id',
					'lkp_cities.city_name','airdom_buyer_quotes.from_location_id',
					'airdom_buyer_quote_items.lkp_load_type_id','airdom_buyer_quotes.lkp_post_status_id',
					'airdom_buyer_quote_items.lkp_packaging_type_id',
					'airdom_buyer_quote_items.calculated_volume_weight','airdom_buyer_quotes.is_door_pickup','airdom_buyer_quotes.is_door_delivery',
					'airdom_buyer_quote_items.units','airdom_buyer_quote_items.lkp_ict_weight_uom_id',
					'airdom_buyer_quote_items.number_packages','lkp_load_types.load_type','lkp_ptl_pincodes.postoffice_name',
					'lkp_ptl_pincodes.pincode','lkp_packaging_types.packaging_type_name','lkp_ict_weight_uom.weight_type'
			)
			->groupBy('airdom_buyer_quotes.id')
			->get();
				
			
			for($i=0;$i<count($buyerquoteid);$i++){
				$buyersquotes	= DB::table('airdom_buyer_quote_sellers_quotes_prices')
				->where('airdom_buyer_quote_sellers_quotes_prices.buyer_quote_id',$buyerquoteid[$i]->ptlquoteid)
				->where('airdom_buyer_quote_sellers_quotes_prices.buyer_id',$buyerquoteid[$i]->buyer_id)
				->where('airdom_buyer_quote_sellers_quotes_prices.seller_id',Auth::user()->id)
				->select('airdom_buyer_quote_sellers_quotes_prices.*')
				->get();
			...
			$buyerpublicquotedetails[]    = DB::table('airdom_buyer_quotes')
			->leftjoin('airdom_buyer_quote_items','airdom_buyer_quote_items.buyer_quote_id','=','airdom_buyer_quotes.id')
			->leftjoin('users','users.id','=','airdom_buyer_quotes.created_by')
			->leftjoin('lkp_load_types','lkp_load_types.id','=','airdom_buyer_quote_items.lkp_load_type_id')
			->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airdom_buyer_quotes.from_location_id')
			...
			->leftjoin('lkp_cities','lkp_cities.id','=','airdom_buyer_quotes.from_location_id')
			->whereIn('airdom_buyer_quotes.id',$matchedIds)
			->select('airdom_buyer_quotes.id as ptlquoteid','airdom_buyer_quote_items.id','airdom_buyer_quote_items.created_by as buyer_id','users.username','airdom_buyer_quotes.dispatch_date',
					'airdom_buyer_quotes.delivery_date','airdom_buyer_quote_items.lkp_quote_price_type_id',
					'airdom_buyer_quotes.from_location_id','airdom_buyer_quotes.to_location_id',
					'lkp_cities.city_name','airdom_buyer_quotes.from_location_id',
					'airdom_buyer_quote_items.lkp_load_type_id','airdom_buyer_quotes.lkp_post_status_id',
			...
			->groupBy('airdom_buyer_quote_items.id')
			->get();
			...
			if($seller_post_items[0]->is_private == 1){
				...
				$buyer_quote_items_leads_data = SellerMatchingComponent::getSellerLeads(AIR_DOMESTIC, $id);
			...
			if(!empty($matchedLeadsIds) && count($matchedLeadsIds) > 0){
				...
				$buyerleadquoteid    = DB::table('airdom_buyer_quotes')
				->leftjoin('airdom_buyer_quote_items','airdom_buyer_quote_items.buyer_quote_id','=','airdom_buyer_quotes.id')
				->leftjoin('users','users.id','=','airdom_buyer_quotes.created_by')
				->leftjoin('lkp_load_types','lkp_load_types.id','=','airdom_buyer_quote_items.lkp_load_type_id')
				->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airdom_buyer_quotes.from_location_id')
				...
				->leftjoin('lkp_ict_weight_uom','lkp_ict_weight_uom.id','=','airdom_buyer_quote_items.lkp_ict_weight_uom_id')
				->leftjoin('lkp_cities','lkp_cities.id','=','airdom_buyer_quotes.from_location_id')
				->whereIn('airdom_buyer_quotes.id',$matchedLeadsIds)
				->select('airdom_buyer_quotes.transaction_id as transaction_no','airdom_buyer_quotes.id as ptlquoteid','airdom_buyer_quote_items.id','airdom_buyer_quote_items.created_by as buyer_id','users.username','airdom_buyer_quotes.dispatch_date',
						'airdom_buyer_quotes.delivery_date','airdom_buyer_quote_items.lkp_quote_price_type_id',
						'airdom_buyer_quotes.from_location_id','airdom_buyer_quotes.to_location_id',
						'lkp_cities.city_name','airdom_buyer_quotes.from_location_id',
				...
				->groupBy('airdom_buyer_quotes.id')
				->orderBy('airdom_buyer_quotes.dispatch_date', 'asc')
				->get();
			
					
				for($i=0;$i<count($buyerleadquoteid);$i++){
					$buyersquotes	= DB::table('airdom_buyer_quote_sellers_quotes_prices')
					->where('airdom_buyer_quote_sellers_quotes_prices.buyer_quote_id',$buyerleadquoteid[$i]->ptlquoteid)
					->where('airdom_buyer_quote_sellers_quotes_prices.buyer_id',$buyerleadquoteid[$i]->buyer_id)
					->where('airdom_buyer_quote_sellers_quotes_prices.seller_id',Auth::user()->id)
					->select('airdom_buyer_quote_sellers_quotes_prices.*')
					->get();
				...
				$buyerleadquotedetails[]    = DB::table('airdom_buyer_quotes')
				->leftjoin('airdom_buyer_quote_items','airdom_buyer_quote_items.buyer_quote_id','=','airdom_buyer_quotes.id')
				->leftjoin('users','users.id','=','airdom_buyer_quotes.created_by')
				->leftjoin('lkp_load_types','lkp_load_types.id','=','airdom_buyer_quote_items.lkp_load_type_id')
				->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airdom_buyer_quotes.from_location_id')
				...
				->groupBy('airdom_buyer_quote_items.id')
				->orderBy('airdom_buyer_quote_items.id', 'desc')
				->get();
		...
		if($seller_post[0]->lkp_post_status_id ==1){
			...
			if($seller_post_items[0]->is_private == 1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post[0]->lkp_ptl_post_type_id))
					...
					$total_count = count(SellerMatchingComponent::getMatchedResults(AIR_DOMESTIC, $seller_post_items[0]->id));
				else
					$total_count =0;
				}
		}
		...
		if($seller_post[0]->lkp_post_status_id ==1){
			...
			if($seller_post_items[0]->is_private == 1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post[0]->lkp_ptl_post_type_id)) {
					...
					$lead_count = count(SellerMatchingComponent::getSellerLeads(AIR_DOMESTIC, $seller_post_items[0]->id));
				}else
					$lead_count =0;
			}
		}
		...
		$post_details='<div class="col-md-12 col-sm-12 col-xs-12 padding-none details_block">
			...
			$allMessagesList = MessagesComponent::listMessages(null,POSTENQURYMESSAGETYPE,null,$id);
				...
				$gridtopnav = '<a href="#" class="'.$color1.'" data-showdiv="ftl-seller-messages"><i class="fa fa-envelope-o"></i> Messages<span class="badge">'.$countMessages.'</span></a>
							...
							<a href="#"  class="'.$color4.'" data-showdiv="ftl-seller-documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_domestic).'</span></a>';
				if($seller_post[0]->lkp_access_id == 2 || $seller_post[0]->lkp_access_id == 3)
					$privatebuyers  = DB::table('airdom_seller_sellected_buyers')
					->leftjoin ( 'airdom_seller_posts', 'airdom_seller_posts.id', '=', 'airdom_seller_sellected_buyers.seller_post_id' )
					->leftjoin('users','users.id','=','airdom_seller_sellected_buyers.buyer_id')
					->where('airdom_seller_sellected_buyers.created_by',Auth::user()->id)
					->where('airdom_seller_sellected_buyers.seller_post_id',$sellerpostid[0]->id)
					->select('users.username')
					->get();
				else
					$privatebuyers =array();
        ...
        /**
	 * AirDom Buyer Private Posts List Page - Grid and filters
	 ...
	 */
	public static function listAIRDOMBuyerPrivatePosts($statusId, $serviceId, $roleId,$type) {
		...
		// query to retrieve buyer posts list and bind it to the grid
                ...
                $Query->leftjoin ( 'airdom_buyer_quote_selected_sellers as bqss', 'bqss.buyer_quote_id', '=', 'ptlbq.id' );
		...
		foreach ( $postResults as $post ) {
			//$buyer_quotes = DB::table ( 'airdom_buyer_quote_items' )->where ( 'buyer_quote_id', $post->id )->select ( 'airdom_buyer_quote_items.*')->get ();
		...
		$grid->row ( function ($row) {
			...
			$buyerdetailsvalue = DB::table ( 'airdom_buyer_quote_items as bqi' )
			...
			$locationids = DB::table('airdom_buyer_quotes as bq')
			->leftjoin('airdom_buyer_quote_items as bqi','bqi.buyer_quote_id','=','bq.id')
			->where('bqi.id','=',$buyer_quote_id)
			...
			$row->cells [5]->value = $buyer_post_status;
							...
							$quoteid = DB::table('airdom_buyer_quote_items as bqi')
							->where('bqi.id','=',$buyer_quote_id)
							->select('bqi.buyer_quote_id')
							->get();
							
							$quoteitems = DB::table('airdom_buyer_quote_items as bqi')
							...
							<div class="col-md-12 padding-none">
								...
								<div class="col-md-3 form-control-fld padding-left-none form-control-fld padding-top">
									...
									placeholder="Rate per Kg *" class="form-control form-control1 numberVal fourdigitstwodecimals_deciVal" value="'.$getInitialQuotePrice[0]->initial_rate_per_kg.'" ></div>'; 
									else
										$row->cells [5]->value .= '
								<div class="col-md-3 form-control-fld padding-left-none form-control-fld padding-top">
								<input type="text" name="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'" 
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'" 
                                    id="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'"
									placeholder="Rate per Kg *" class="ptl_initial_rate_per_kg form-control  form-control1 fourdigitstwodecimals_deciVal" ></div>';
								...
								<div class="col-md-3 form-control-fld padding-left-none form-control-fld padding-top">
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_initial_conversion fourdigitsfourdecimals_deciVal form-control  form-control1" >';
								...
								elseif(isset($getFinalQuotePrice[0]->final_kg_per_cft) && $getFinalQuotePrice[0]->final_kg_per_cft=='' && $getInitialQuotePrice[0]->initial_kg_per_cft!=''  &&  $getCounterQuotePrice[0]->counter_rate_per_kg!='')
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_final_conversion form-control form-control1 numberVal fourdigitsfourdecimals_deciVal" ></div>
								...
								if(isset($getInitialQuotePrice[0]->initial_quote_price) && $getInitialQuotePrice[0]->initial_quote_price=='0.0000'){
									if($dpickup == 1){
			                            ...
			                            data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"  
			                            id="initial_quote_pickup_'.$buyer_id.'_'.$buyer_quote_id.'" 
	                                    placeholder="Pickup Rs *" class="ptl_initial_pickup form-control form-control1 numberVal fourdigitstwodecimals_deciVal">';
									}else{
										$row->cells [5]->value .= '
										<input type="text"
	                                    ...
	                                    placeholder="No Pickup Rs *" class="form-control form-control1 numberVal fourdigitstwodecimals_deciVal" readonly>
			                            <input type="hidden"
	                                    name="initial_quote_pickup_'.$buyer_id.'_'.$buyer_quote_id.'"
			                            data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
			                            id="initial_quote_pickup_'.$buyer_id.'_'.$buyer_quote_id.'"
	                                    placeholder="Pickup Rs *" class="ptl_initial_pickup form-control form-control1 numberVal fourdigitstwodecimals_deciVal" value="0">';
									}
								}
								elseif(isset($getCounterQuotePrice[0]->counter_quote_price) && $getCounterQuotePrice[0]->counter_quote_price !='0.0000' && isset($getFinalQuotePrice[0]->final_quote_price) && $getFinalQuotePrice[0]->final_quote_price=='0.0000'){
									if($dpickup == 1){
                	                    ...
                	                    placeholder="Pickup Rs *" class="ptl_final_pickup form-control form-control1 numberVal fourdigitstwodecimals_deciVal">';
									}else{
										$row->cells [5]->value .= '
										<input type="text"
			                            ...
			                            data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
		    	                        id="final_quote_pickup_'.$buyer_id.'_'.$buyer_quote_id.'"
                	                    placeholder="Pickup Rs *" class="ptl_final_pickup form-control form-control1 numberVal fourdigitstwodecimals_deciVal " value="0">';
									}
								}
								...
								if(isset($getInitialQuotePrice[0]->initial_quote_price) && $getInitialQuotePrice[0]->initial_quote_price=='0.0000'){
									if($ddelivery == 1){
	                                    ...
	                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
	                                    id="initial_quote_delivery_'.$buyer_id.'_'.$buyer_quote_id.'" 
			                            placeholder="Delivery Rs *" class="ptl_initial_delivery form-control form-control1 numberVal fourdigitstwodecimals_deciVal">';
									}else{
										$row->cells [5]->value .='<input type="text"
										name="initial_quote_delivery"
	                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
	                                    id="initial_quote_delivery"
			                            placeholder="No Delivery Rs *" class="form-control form-control1 numberVal fourdigitstwodecimals_deciVal" readonly>
	                                    		<input type="hidden"
										name="initial_quote_delivery_'.$buyer_id.'_'.$buyer_quote_id.'"
	                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
	                                    id="initial_quote_delivery_'.$buyer_id.'_'.$buyer_quote_id.'"
			                            placeholder="Delivery Rs *" class="ptl_initial_delivery form-control form-control1 numberVal fourdigitstwodecimals_deciVal" value="0">';
									}
								}
								elseif(isset($getCounterQuotePrice[0]->counter_quote_price) && $getCounterQuotePrice[0]->counter_quote_price !='0.0000' && isset($getFinalQuotePrice[0]->final_quote_price) && $getFinalQuotePrice[0]->final_quote_price=='0.0000'){
									if($ddelivery == 1){
	                                    ...
	                                    placeholder="Delivery Rs *" class="ptl_final_delivery form-control form-control1 numberVal fourdigitstwodecimals_deciVal ">';
									}else{
										$row->cells [5]->value .= '
										<input type="text"
			                            ...
			                            data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
			                            id="final_quote_delivery_'.$buyer_id.'_'.$buyer_quote_id.'"
	                                    placeholder="Delivery Rs *" class="ptl_final_delivery form-control form-control1 numberVal fourdigitstwodecimals_deciVal " value="0">';
								...
								if(isset($getInitialQuotePrice[0]->initial_quote_price) && $getInitialQuotePrice[0]->initial_quote_price=='0.0000'){
                                    ...
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'" 
                                    id="initial_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'" 
                                    placeholder="ODA Rs *" class="ptl_initial_oda form-control form-control1 numberVal fourdigitstwodecimals_deciVal">';
									}else{
										$row->cells [5]->value .= '<input type="text"
									name="initial_quote_oda"
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
                                    id="initial_quote_oda"
                                    placeholder="No ODA Rs *" class="form-control form-control1 numberVal fourdigitstwodecimals_deciVal" readonly>
                                    		<input type="hidden"
									name="initial_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'"
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
                                    id="initial_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'"
                                    placeholder="ODA Rs *" class="ptl_initial_oda form-control form-control1 numberVal fourdigitstwodecimals_deciVal " Value="0">';
									}
								}
								elseif(isset($getCounterQuotePrice[0]->counter_quote_price) && $getCounterQuotePrice[0]->counter_quote_price !='0.0000' && isset($getFinalQuotePrice[0]->final_quote_price) && $getFinalQuotePrice[0]->final_quote_price=='0.0000'){
									if($odacheck == 1){
	                                    ...
	                                    placeholder="ODA Rs *" class="ptl_final_oda form-control form-control1 numberVal fourdigitstwodecimals_deciVal ">';
									}else{
										$row->cells [5]->value .= '
									<input type="text"
								...
								if(isset($getInitialQuotePrice[0]->initial_transit_days) && $getInitialQuotePrice[0]->initial_transit_days=='')
                                    ...
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"  
                                    id="initial_quote_transit_'.$buyer_id.'_'.$buyer_quote_id.'"
                                    placeholder="Transit Days *" class="form-control form-control1 numberVal " maxlength="2">';
								...
								$row->cells [5]->value .= '<input type="text"
                                    ...
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
                                    id="initial_quote_transit_'.$buyer_id.'_'.$buyer_quote_id.'"
                                    placeholder="Transit Days *" class="form-control form-control1 numberVal " maxlength="">';
								$row->cells [5]->value .= '</div>';
	...
	public static function listAirdomBuyerMarketLeads($statusId, $roleId, $serviceId, $id){
		try{
