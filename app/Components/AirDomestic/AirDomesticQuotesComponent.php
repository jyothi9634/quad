...
class AirDomesticQuotesComponent {
	
	/**
	 * Submitting Seller Initial Quoted
	 *	
	 * @param  $request
	 * @return Response
	 */
	public static function AirDomesticSellerQuoteSubmit() {
		try{
			...
			//echo "asdf";exit;
			$getSellerpostdetails  = DB::table('airdom_seller_post_items')
				->leftjoin('airdom_seller_posts','airdom_seller_posts.id','=','airdom_seller_post_items.seller_post_id')
				->where('airdom_seller_post_items.id','=',Session::get('seller_post_item'))
				->where('airdom_seller_post_items.created_by','=',Auth::user()->id)
				->select('airdom_seller_posts.*','airdom_seller_post_items.*')
				->get();
				
			
				$getBuyerpostdetails  = DB::table('airdom_buyer_quotes')
				->where('airdom_buyer_quotes.id','=',$buyerQuoteItemId)
				->where('airdom_buyer_quotes.created_by','=',$buyerId)
				->select('airdom_buyer_quotes.*')
				->get();
				...
				$createsellerpost = new AirdomSellerPost();
				$createsellerpost->lkp_service_id = AIR_DOMESTIC;
				...
				$createsellerpostitem = new AirdomSellerPostItem();
				...
				$sellerselectedbuyer = new AirdomSellerSellectedBuyer();
				...
				$getbqsqp = DB::table('airdom_buyer_quote_sellers_quotes_prices as bqsp')
                ...
                if(count($getbqsqp)>0){
                	...
                	DB::table('airdom_buyer_quote_sellers_quotes_prices')
                	->where('airdom_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
                	->where('airdom_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$_REQUEST['cbuyerquoteid'])
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$sellerinitialquote = new AirdomBuyerQuoteSellersQuotesPrice();
	 ...
	 * @return Response
	 */
	public static function AirdomesticSellerFinalQuoteSubmit() {
		try{
			$formvalues = urldecode($_REQUEST['formvalues']);
			$formfields = explode("&", $formvalues);
			$hiddenfields = array();
			...
			CommonComponent::auditLog($buyerId,'airdom_buyer_quote_sellers_quotes_prices');
	 ...
	 * @return Response
	 */
	public static function AirDomesticsellerSearchQuoteSubmit() {
		try{
			...
			$getBuyerpostdetails  = DB::table('airdom_buyer_quote_items')
			->leftjoin('airdom_buyer_quotes', 'airdom_buyer_quotes.id', '=', 'airdom_buyer_quote_items.buyer_quote_id')
			->where('airdom_buyer_quote_items.id','=',$buyerQuoteItemId)
			->where('airdom_buyer_quotes.created_by','=',$buyerId)
			->select('airdom_buyer_quotes.*')
			->get();
             ...
             if(count($getbqsqp)>0){
              ...
              DB::table('airdom_buyer_quote_sellers_quotes_prices')
               ...
               ->update(array('initial_quote_price' =>$total_final_amount,
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$buyerinitial = new AirdomBuyerQuoteSellersQuotesPrice();
	 ...
	 * @return Response
	 */
	public static function AirdomesticsellerCounterAcceptance() {
		try{
			...
			$getcounter = DB::table('airdom_buyer_quote_sellers_quotes_prices')
			->where('airdom_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
			->where('airdom_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$buyerQuoteItemId)
			//->where('airdom_buyer_quote_sellers_quotes_prices.seller_post_item_id','=',$sellerInput['seller_post_item_id'])
			->where('airdom_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
