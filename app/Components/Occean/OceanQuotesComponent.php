...
class OceanQuotesComponent {
	 ...
	 * @return Response
	 */
	public static function OceanSellerQuoteSubmit($request) {
		try{
			...
			if(isset($sellerInput['buyer_buyerquote_id']) && !empty($sellerInput['buyer_buyerquote_id'])) {
				...
				$getSellerpostdetails  = DB::table('ocean_seller_post_items')
				->leftjoin('ocean_seller_posts','ocean_seller_posts.id','=','ocean_seller_post_items.seller_post_id')
				->where('ocean_seller_post_items.id','=',Session::get('seller_post_item'))
				->where('ocean_seller_post_items.created_by','=',Auth::user()->id)
				->select('ocean_seller_posts.*','ocean_seller_post_items.*')
				->get();
				
					
				$getBuyerpostdetails  = DB::table('ocean_buyer_quotes')
				->where('ocean_buyer_quotes.id','=',$buyerQuoteItemId)
				->where('ocean_buyer_quotes.created_by','=',$buyerId)
				->select('ocean_buyer_quotes.*')
				->get();
				...
				$createsellerpost = new OceanSellerPost();
				$createsellerpost->lkp_service_id = OCEAN;
				...
				$createsellerpostitem = new OceanSellerPostItem();
				...
				$sellerselectedbuyer = new OceanSellerSellectedBuyer();
			...
			//code added by swathi for updating values from market leads	
                        $getbqsqp = DB::table('ocean_buyer_quote_sellers_quotes_prices as bqsp')
                        ...
                        if(count($getbqsqp)>0){
                               ...
                               DB::table('ocean_buyer_quote_sellers_quotes_prices')
                                ->where('ocean_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
                                ->where('ocean_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$_REQUEST['cbuyerquoteid'])
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$sellerinitialquote = new OceanBuyerQuoteSellersQuotesPrice();
	 ...
	 * @return Response
	 */
	public static function OceanSellerFinalQuoteSubmit($request) {
		try{
			$formvalues = urldecode($_REQUEST['formvalues']);
			$formfields = explode("&", $formvalues);
			$hiddenfields = array();
			...
			CommonComponent::auditLog($buyerId,'ocean_buyer_quote_sellers_quotes_prices');
	 ...
	 * @return Response
	 */
	public static function OceansellerSearchQuoteSubmit($request) {
		try{
			...
			$getBuyerpostdetails  = DB::table('ocean_buyer_quote_items')
			->leftjoin('ocean_buyer_quotes', 'ocean_buyer_quotes.id', '=', 'ocean_buyer_quote_items.buyer_quote_id')
			->where('ocean_buyer_quote_items.id','=',$buyerQuoteItemId)
			->where('ocean_buyer_quotes.created_by','=',$buyerId)
			->select('ocean_buyer_quotes.*')
			->get();
				if(count($getBuyerpostdetails)>0){
			...
			$total_final_amount = $totalfrieghtamount;
				...
				$fromloc = CommonComponent::getOceanId($sellerInput['from_city_loc']);
				$toloc = CommonComponent::getOceanId($sellerInput['to_city_loc']);
			...
			//code added by swathi for updating values from market leads	
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$buyerinitial = new OceanBuyerQuoteSellersQuotesPrice();
	 ...
	 * @return Response
	 */
	public static function OceansellerCounterAcceptance($request) {
		try{
			...
			$getcounter = DB::table('ocean_buyer_quote_sellers_quotes_prices')
			->where('ocean_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
			->where('ocean_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$buyerQuoteItemId)
			//->where('ocean_buyer_quote_sellers_quotes_prices.seller_post_item_id','=',$sellerInput['seller_post_item_id'])
			->where('ocean_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
	 ...
	 * @return Response
	 */
	public static function OceansellerQuoteAcceptance($id,$bqid,$spqi=null) {
		try{
