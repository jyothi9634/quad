...
class AirInternationalSellerListingComponent {
	 ...
	 * @return Response
	 */
	public static function AirInternationalSellerList($statusId, $roleId, $serviceId) {
		...
		if(isset($_REQUEST['lead_name']) && ($_REQUEST['lead_name'] ==2)){
			...
			Session::put('leads', '1');
			$Query->leftjoin ( 'airint_buyer_quote_selected_sellers as bqss', 'bqss.seller_id', '=', 'pspi.created_by' );
		}
		...
		foreach($sellerresults as $seller){
			...
			foreach($seller_post_items as $seller_post_item){
				if(!isset($from_locations[$seller_post_item->from_location_id])){
					$from_locations[$seller_post_item->from_location_id] = DB::table('lkp_airports')->where('id', $seller_post_item->from_location_id)->pluck('airport_name');
				}
				if(!isset($to_locations[$seller_post_item->to_location_id])){
					$to_locations[$seller_post_item->to_location_id] = DB::table('lkp_airports')->where('id', $seller_post_item->to_location_id)->pluck('airport_name');
				}
			}
		}
		...
		$grid->row ( function ($row) {
			...
			->where('airint_seller_post_items.seller_post_id','=',$spId)
			->select('airint_seller_post_items.*')
				->get();
			...
			$Ptlseller_post_items  = DB::table('airint_seller_post_items')
			...
			$docs_seller_international    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$spId);
			...
			->join ( 'airint_seller_posts', 'airint_seller_posts.id', '=', 'airint_seller_post_items.seller_post_id' )
			->where('airint_seller_post_items.seller_post_id',$spId)
			->select('airint_seller_post_items.*','airint_seller_posts.lkp_ptl_post_type_id','airint_seller_post_items.from_location_id','airint_seller_post_items.to_location_id')
			->get();
			$total_count=0;
			if($privatepost==1){
			
				$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(AIR_INTERNATIONAL,$seller_post_items[0]->seller_post_id));
				...
				for($k=0;$k<count($seller_post_items);$k++)
					$total_count += count(SellerMatchingComponent::getMatchedResults(AIR_INTERNATIONAL, $seller_post_items[$k]->id));
				}
				else 
					$total_count =0;
			}
			...
			$row->cells [5]->value .='
							...
							<a><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_international).'</span></a>
						</div>
					</div>
	...
	public static function listAirintSellerPostItems($statusId, $roleId, $serviceId, $id){
		try{
			...
			$lkppoststatus  = DB::table('airint_seller_posts')
			->where('airint_seller_posts.id','=',$id)
			->select('airint_seller_posts.lkp_ptl_post_type_id')
					->get();
			
			Session::put('lkppoststatus', $lkppoststatus[0]->lkp_ptl_post_type_id);
			$Query = DB::table ( 'airint_seller_posts as sp' );
			$Query->leftjoin ( 'airint_seller_post_items as spi', 'spi.seller_post_id', '=', 'sp.id' );
			$Query->leftjoin ( 'lkp_post_statuses as ps', 'ps.id', '=', 'sp.lkp_post_status_id' );
			$Query->leftjoin('lkp_airports','lkp_airports.id','=','spi.from_location_id');
			...
			if(Session::get('leads') &&  Session::get('leads')==2){
				...
				$Query->leftjoin ( 'airint_buyer_quote_sellers_quotes_prices as bqss', 'bqss.seller_id', '=', 'spi.created_by' );
			}
			...
			$sellerresults = $Query->select ( 'spi.id', 'spi.from_location_id','spi.to_location_id','spi.price',
					'sp.kg_per_cft', 'spi.transitdays' ,'sp.lkp_post_status_id','lkp_airports.airport_name as postoffice_name',
					'sp.lkp_access_id', 'ps.post_status','sp.id as post_id',
			...
			foreach($sellerresults as $seller){
				$seller_post_items  = DB::table('airint_seller_post_items')
				->where('airint_seller_post_items.id',$seller->id)
				->select('*')
				->get();
				foreach($seller_post_items as $seller_post_item){
					if(!isset($from_locations[$seller_post_item->from_location_id])){
			...
			$grid->add ( 'id', 'ID', true )->style ( "display:none" );
			$grid->add ( 'postoffice_name', 'From Location', 'postoffice_name' )->attributes(array("class" => "col-md-2 col-sm-2 col-xs-5 padding-none"));
			...
			$grid->row ( function ($row) {
				...
				//View Count
				$countview = DB::table('airint_seller_post_item_views')
				->where('airint_seller_post_item_views.seller_post_item_id','=',$spId)
				->select('airint_seller_post_item_views.id','airint_seller_post_item_views.view_counts')
				->get();
				if(!isset($countview[0]->view_counts))
					$countview = 0;
				...
				$row->cells [2]->value = ''.CommonComponent::getAirportName($row->cells [2]->value).'';
				...
				$seller_post_items  = DB::table('airint_seller_post_items')
				->join ( 'airint_seller_posts', 'airint_seller_posts.id', '=', 'airint_seller_post_items.seller_post_id' )
				->where('airint_seller_post_items.id',$spId)
				->select('airint_seller_post_items.*','airint_seller_posts.lkp_ptl_post_type_id','airint_seller_posts.id as spid')
				->get();

				
				//count for seller documents
				$serviceId = Session::get('service_id');
				$docs_seller_international    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$seller_post_items[0]->spid);
				...
				if($seller_post_items[0]->is_private == 1){
					$total_count = count(CommonComponent::getPrivateBuyerMatchedResults(AIR_INTERNATIONAL,$seller_post_items[0]->spid));
				}else{
					if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post_items[0]->lkp_ptl_post_type_id)){
						//$total_count =AirInternationalSellerListingComponent::enquiryCount(2,$seller_post_items[0]->from_location_id,$seller_post_items[0]->to_location_id,$seller_post_items[0]->lkp_ptl_post_type_id );
						$total_count += count(SellerMatchingComponent::getMatchedResults(AIR_INTERNATIONAL, $spId));
					}
					else 
						$total_count =0;
				}
				...
				$row->cells [7]->value .='<div class="clearfix"></div>
								<div class="pull-left">
									<div class="info-links">
										...
										<a href="/sellerpostdetail/'.$spId.'?type=documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_international).'</span></a>
									</div>
								</div>
								<div class="pull-right text-right">
									<div class="info-links">';
	...
	public static function listAirintSellertopNavPostItems($id){
		
		$seller_post_items  = DB::table('airint_seller_posts')
							->where('airint_seller_posts.id',$id)
							->select('airint_seller_posts.id','airint_seller_posts.transaction_id','airint_seller_posts.kg_per_cft',
									 'airint_seller_posts.lkp_access_id','airint_seller_posts.from_date',
									 'airint_seller_posts.to_date','airint_seller_posts.lkp_payment_mode_id',
									 'airint_seller_posts.tracking','airint_seller_posts.accept_payment_netbanking','airint_seller_posts.credit_period','airint_seller_posts.credit_period_units'
									 ,'airint_seller_posts.accept_payment_credit','airint_seller_posts.accept_payment_debit'
									 ,'airint_seller_posts.accept_credit_netbanking','airint_seller_posts.accept_credit_cheque'
									 ,'airint_seller_posts.cancellation_charge_text','airint_seller_posts.cancellation_charge_price'
									 ,'airint_seller_posts.docket_charge_text','airint_seller_posts.docket_charge_price','airint_seller_posts.terms_conditions'
									 ,'airint_seller_posts.other_charge1_text','airint_seller_posts.other_charge1_price'
									 ,'airint_seller_posts.other_charge2_text','airint_seller_posts.other_charge2_price'
									 ,'airint_seller_posts.other_charge3_text','airint_seller_posts.other_charge3_price')
							->get();
		...
		if($seller_post_items[0]->lkp_access_id == 2 || $seller_post_items[0]->lkp_access_id == 3)
			$privatebuyers  = DB::table('airint_seller_sellected_buyers')
			->leftjoin ( 'airint_seller_posts', 'airint_seller_posts.id', '=', 'airint_seller_sellected_buyers.seller_post_id' )
			->leftjoin('users','users.id','=','airint_seller_sellected_buyers.buyer_id')
			->where('airint_seller_sellected_buyers.created_by',Auth::user()->id)
			->where('airint_seller_sellected_buyers.seller_post_id',$id)
	...
	public static function listAirintSellerPostDetailsItems($id){
		Session::put('airint_seller_post_item', $id);
		try{
			$viewcount  = DB::table('airint_seller_post_items')
					->where('airint_seller_post_items.id','=',$id)
					->select('airint_seller_post_items.id',
							'airint_seller_post_items.created_by')
					->get();
			...
			$seller_post = DB::table('airint_seller_posts')
			->join('airint_seller_post_items','airint_seller_post_items.seller_post_id','=','airint_seller_posts.id')
			->where('airint_seller_post_items.id',$id)
			->select('airint_seller_posts.*','airint_seller_post_items.id')
			->get();
			$sellerpostid = DB::table('airint_seller_posts')
			->join('airint_seller_post_items','airint_seller_post_items.seller_post_id','=','airint_seller_posts.id')
			->where('airint_seller_post_items.id',$id)
			->select('airint_seller_posts.id')
			->get();
			
			
			//count for seller documents
			$serviceId = Session::get('service_id');
			$docs_seller_international    =   CommonComponent::getGsaDocuments(SELLER,$serviceId,$sellerpostid);
			...
			$matchedIds = array();
			if($seller_post_items[0]->is_private == 1){
				$matchedIds[] = CommonComponent::getPrivateBuyerMatchedResults(AIR_INTERNATIONAL,$seller_post_items[0]->seller_post_id);
			}
			else{
				$buyer_quote_items_matched_data = SellerMatchingComponent::getMatchedResults(AIR_INTERNATIONAL, $id);
			...
			if(!empty($matchedIds) && count($matchedIds) > 0){
					$buyerdetails   = DB::table('airint_buyer_quotes')
					->leftjoin('airint_buyer_quote_items','airint_buyer_quote_items.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('airint_buyer_quote_selected_sellers','airint_buyer_quote_selected_sellers.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('users','users.id','=','airint_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','airint_buyer_quote_items.lkp_load_type_id')
					->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airint_buyer_quotes.from_location_id')
					->leftjoin('lkp_packaging_types','lkp_packaging_types.id','=','airint_buyer_quote_items.lkp_packaging_type_id')
					->leftjoin('lkp_ict_weight_uom','lkp_ict_weight_uom.id','=','airint_buyer_quote_items.lkp_ict_weight_uom_id')
					->leftjoin('lkp_cities','lkp_cities.id','=','airint_buyer_quotes.from_location_id')
					->leftjoin('airint_buyer_quote_sellers_quotes_prices','airint_buyer_quote_sellers_quotes_prices.buyer_quote_id','=','airint_buyer_quotes.id')
					->whereIn('airint_buyer_quotes.id',$matchedIds)
					->select('airint_buyer_quotes.id as ptlquoteid','airint_buyer_quote_items.id','users.username','airint_buyer_quotes.dispatch_date',
							'airint_buyer_quotes.delivery_date','airint_buyer_quote_items.lkp_quote_price_type_id',
							'airint_buyer_quotes.from_location_id','airint_buyer_quotes.to_location_id',
							'airint_buyer_quote_items.created_by',
							'lkp_cities.city_name','airint_buyer_quotes.lkp_post_status_id',
							'airint_buyer_quote_sellers_quotes_prices.initial_quote_price',
							'airint_buyer_quote_sellers_quotes_prices.counter_quote_price',
							'airint_buyer_quote_sellers_quotes_prices.final_quote_price',
							'airint_buyer_quote_sellers_quotes_prices.buyer_id',
							'airint_buyer_quote_sellers_quotes_prices.seller_acceptence',
							'airint_buyer_quote_sellers_quotes_prices.initial_freight_amount',
							'airint_buyer_quote_sellers_quotes_prices.counter_freight_amount',
							'airint_buyer_quote_sellers_quotes_prices.final_freight_amount',
							'airint_buyer_quote_sellers_quotes_prices.initial_rate_per_kg',
							'airint_buyer_quote_sellers_quotes_prices.counter_rate_per_kg',
							'airint_buyer_quote_sellers_quotes_prices.final_rate_per_kg',
							'airint_buyer_quote_sellers_quotes_prices.initial_kg_per_cft',
							'airint_buyer_quote_sellers_quotes_prices.counter_kg_per_cft',
							'airint_buyer_quote_sellers_quotes_prices.final_kg_per_cft',
							'airint_buyer_quote_sellers_quotes_prices.initial_pick_up_rupees',
							'airint_buyer_quote_sellers_quotes_prices.initial_delivery_rupees',
							'airint_buyer_quote_sellers_quotes_prices.initial_oda_rupees',
							'airint_buyer_quote_sellers_quotes_prices.initial_transit_days',
							'airint_buyer_quote_sellers_quotes_prices.final_pick_up_rupees',
							'airint_buyer_quote_sellers_quotes_prices.final_delivery_rupees',
							'airint_buyer_quote_sellers_quotes_prices.final_oda_rupees',
							'airint_buyer_quote_sellers_quotes_prices.final_transit_days',
							'airint_buyer_quote_sellers_quotes_prices.id as bqsqpid',
							'airint_buyer_quotes.from_location_id',
							'airint_buyer_quote_items.lkp_load_type_id',
							'airint_buyer_quote_items.lkp_packaging_type_id',
							'airint_buyer_quote_items.calculated_volume_weight',
							'airint_buyer_quote_items.units','airint_buyer_quote_items.lkp_ict_weight_uom_id',
							'airint_buyer_quote_items.number_packages','lkp_load_types.load_type','lkp_ptl_pincodes.postoffice_name',
							'lkp_ptl_pincodes.pincode','lkp_packaging_types.packaging_type_name','lkp_ict_weight_uom.weight_type')
							->groupBy('airint_buyer_quotes.id')
							->get();
					
					$buyerprivatequotedetails = array();
					$buyerprivatequotedetails[]    = DB::table('airint_buyer_quotes')
					->leftjoin('airint_buyer_quote_items','airint_buyer_quote_items.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('airint_buyer_quote_selected_sellers','airint_buyer_quote_selected_sellers.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('users','users.id','=','airint_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','airint_buyer_quote_items.lkp_load_type_id')
					...
					->select('airint_buyer_quotes.id as ptlquoteid','airint_buyer_quote_items.id','users.username','airint_buyer_quotes.dispatch_date',
							...
							'airint_buyer_quote_items.number_packages','lkp_load_types.load_type','lkp_ptl_pincodes.postoffice_name',
							'lkp_ptl_pincodes.pincode','lkp_packaging_types.packaging_type_name','lkp_ict_weight_uom.weight_type')
							->groupBy('airint_buyer_quote_items.id')
							->get();
					
					
					$buyerquoteid    = DB::table('airint_buyer_quotes')
					->leftjoin('airint_buyer_quote_items','airint_buyer_quote_items.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('users','users.id','=','airint_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','airint_buyer_quote_items.lkp_load_type_id')
					->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airint_buyer_quotes.from_location_id')
					...
					->leftjoin('lkp_cities','lkp_cities.id','=','airint_buyer_quotes.from_location_id')
					->whereIn('airint_buyer_quotes.id',$matchedIds)
					->select('airint_buyer_quotes.transaction_id as transaction_no','airint_buyer_quotes.id as ptlquoteid','airint_buyer_quote_items.id','airint_buyer_quote_items.created_by as buyer_id','users.username','airint_buyer_quotes.dispatch_date',
							'airint_buyer_quotes.delivery_date','airint_buyer_quote_items.lkp_quote_price_type_id',
							'airint_buyer_quotes.from_location_id','airint_buyer_quotes.to_location_id',
							'lkp_cities.city_name','airint_buyer_quotes.from_location_id',
							'airint_buyer_quote_items.lkp_load_type_id','airint_buyer_quotes.lkp_post_status_id',
							'airint_buyer_quote_items.lkp_packaging_type_id',
							'airint_buyer_quote_items.calculated_volume_weight',
							'airint_buyer_quote_items.units','airint_buyer_quote_items.lkp_ict_weight_uom_id',
							'airint_buyer_quote_items.number_packages','lkp_load_types.load_type','lkp_ptl_pincodes.postoffice_name',
					...
					$buyerpublicquotedetails[]    = DB::table('airint_buyer_quotes')
					->leftjoin('airint_buyer_quote_items','airint_buyer_quote_items.buyer_quote_id','=','airint_buyer_quotes.id')
					->leftjoin('users','users.id','=','airint_buyer_quotes.created_by')
					->leftjoin('lkp_load_types','lkp_load_types.id','=','airint_buyer_quote_items.lkp_load_type_id')
					->leftjoin('lkp_ptl_pincodes','lkp_ptl_pincodes.id','=','airint_buyer_quotes.from_location_id')
					...
					->leftjoin('lkp_cities','lkp_cities.id','=','airint_buyer_quotes.from_location_id')
					->whereIn('airint_buyer_quotes.id',$matchedIds)
					->select('airint_buyer_quotes.id as ptlquoteid','airint_buyer_quote_items.id','airint_buyer_quote_items.created_by as buyer_id','users.username','airint_buyer_quotes.dispatch_date',
							'airint_buyer_quotes.delivery_date','airint_buyer_quote_items.lkp_quote_price_type_id',
							'airint_buyer_quotes.from_location_id','airint_buyer_quotes.to_location_id',
							'lkp_cities.city_name','airint_buyer_quotes.from_location_id',
							'airint_buyer_quote_items.lkp_load_type_id','airint_buyer_quotes.lkp_post_status_id',
				...
				for($i=0;$i<count($buyerquoteid);$i++){
					$buyersquotes	= DB::table('airint_buyer_quote_sellers_quotes_prices')
					->where('airint_buyer_quote_sellers_quotes_prices.buyer_quote_id',$buyerquoteid[$i]->ptlquoteid)
					->where('airint_buyer_quote_sellers_quotes_prices.buyer_id',$buyerquoteid[$i]->buyer_id)
		...
		if($seller_post[0]->lkp_post_status_id ==1){
			...
			if($seller_post_items[0]->is_private == 1){
				...
				if(isset($seller_post_items[0]->from_location_id) && isset($seller_post_items[0]->to_location_id) && isset($seller_post[0]->lkp_ptl_post_type_id))
					...
					$total_count = count(SellerMatchingComponent::getMatchedResults(AIR_INTERNATIONAL, $seller_post_items[0]->id));
				else
					$total_count =0;
			}
		}
		...
		$post_details='<div class="col-md-12 col-sm-12 col-xs-12 padding-none details_block">
							...
							<h5>
								<div class="col-md-4 col-sm-5 col-xs-6 padding-none"><b>Air International</b></div>
		...
		$allMessagesList = MessagesComponent::listMessages(null,POSTENQURYMESSAGETYPE,null,$id);
				...
				$gridtopnav = '<a href="#" class="'.$color1.'"  data-showdiv="ftl-seller-messages" title="messages"><i class="fa fa-envelope-o"></i> Messages<span class="badge">'.$countMessages.'</span></a>
							...
							<a href="#" class="'.$color4.'" data-showdiv="ftl-seller-documentation"><i class="fa fa-file-text-o"></i> Documentation <span class="badge">'.count($docs_seller_international).'</span></a>';
		...
		if($seller_post[0]->lkp_access_id == 2 || $seller_post[0]->lkp_access_id == 3)
			...
			->leftjoin('users','users.id','=','airint_seller_sellected_buyers.buyer_id')
			->where('airint_seller_sellected_buyers.created_by',Auth::user()->id)
			->where('airint_seller_sellected_buyers.seller_post_id',$sellerpostid[0]->id)
			->select('users.username')
			->get();
		else
			$privatebuyers =array();
	 ...
	 */
	public static function listAIRINTBuyerPrivatePosts($statusId, $serviceId, $roleId,$type) {
		...
		// query to retrieve buyer posts list and bind it to the grid
                ...
                $Query->leftjoin ( 'airint_buyer_quote_selected_sellers as bqss', 'bqss.buyer_quote_id', '=', 'ptlbq.id' );
		...
		$postResults = $Query->select ('ptlbqi.id','ptlbq.dispatch_date','ptlbq.delivery_date','ptlbq.is_dispatch_flexible','ptlbq.from_location_id',
                       ...
                       ,'ptlPins.airport_name as frompincode','ptlPins.id as fromairportid' ,'ptlPinsTo.airport_name as topincode','ptlPinsTo.id as toairportid'
		...
		foreach ( $postResults as $post ) {
			//$buyer_quotes = DB::table ( 'airint_buyer_quote_items' )->where ( 'buyer_quote_id', $post->id )->select ( 'airint_buyer_quote_items.*')->get ();
				
			//foreach ( $buyer_quotes as $quotes ) {
				if (! isset ( $from_locations [$post->from_location_id] )) {
					$from_locations [$post->from_location_id] = DB::table ( 'lkp_airports' )->where ( 'id', $post->from_location_id )->pluck ( 'airport_name' );
				}
				if (! isset ( $to_locations [$post->to_location_id] )) {
					$to_locations [$post->to_location_id] = DB::table ( 'lkp_airports' )->where ( 'id', $post->to_location_id )->pluck ( 'airport_name' );
				}
							...
							<div class="col-md-12 col-sm-12 col-xs-12 padding-none margin-top ">
								...
								if(isset($getCounterQuotePrice[0]->counter_rate_per_kg) && $getCounterQuotePrice[0]->counter_rate_per_kg !='')
										$row->cells [5]->value .='
											...
											placeholder="Conversion Kg/CCM *" class="form-control form-control1 numberVal  margin-top"  value="'.$getCounterQuotePrice[0]->counter_rate_per_kg.'">
													'; 
								...
								if(isset($getCounterQuotePrice[0]->counter_kg_per_cft) && $getCounterQuotePrice[0]->counter_kg_per_cft!=''){
										...
										<input type="hidden"
											...
											placeholder="Conversion Kg/CCM *" class="form-control form-control1 numberVal  margin-top"  value="'.$getCounterQuotePrice[0]->counter_kg_per_cft.'">
	...
	public static function listAirintBuyerMarketLeads($statusId, $roleId, $serviceId, $id){
		try{
			//Filters values to populate in the page
			$from_locations = array(""=>"From Location");
			$to_locations = array(""=>"To Location");
