...
class RailSellerComponent {
	public static function getRailSellerSearchList($roleId, $serviceId, $statusId) {
		// print_R($_GET);exit;
...
//		echo "From - ".$_REQUEST ['from_location_id']." To - ".$_REQUEST ['to_location_id'];
		...
		$grid->row ( function ($row) {
			...
			$buyerdetailsvalue = DB::table ( 'rail_buyer_quote_items' )->where ( 'rail_buyer_quote_items.id', '=', $buyer_quote_id )->select ( 'rail_buyer_quote_items.*' )->get ();
			...
			$getSellerpost  = DB::table('rail_seller_post_items')
			->join( 'rail_seller_posts', 'rail_seller_posts.id', '=', 'rail_seller_post_items.seller_post_id' )
			->join( 'rail_buyer_quote_sellers_quotes_prices', 'rail_buyer_quote_sellers_quotes_prices.seller_post_item_id', '=', 'rail_seller_post_items.id' )
			->where('rail_seller_post_items.from_location_id','=',$from_zipcode)
			->where('rail_seller_post_items.to_location_id','=',$to_zipcode)
			->where('rail_seller_post_items.created_by','=',Auth::user()->id)
			->where('rail_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$bqid)
			->where('rail_seller_posts.lkp_post_status_id','=',OPEN)
			->select('rail_seller_post_items.seller_post_id',
					'rail_seller_post_items.id',
					'rail_seller_posts.tracking',
					'rail_seller_posts.lkp_payment_mode_id',
					'rail_seller_posts.accept_payment_netbanking',
					'rail_seller_posts.accept_payment_credit',
					'rail_seller_posts.accept_payment_debit',
					'rail_seller_posts.credit_period',
					'rail_seller_posts.credit_period_units',
					'rail_seller_posts.accept_credit_netbanking',
					'rail_seller_posts.accept_credit_cheque')
					->get();
			...
			$locationids = DB::table('rail_buyer_quotes')
			->leftjoin('rail_buyer_quote_items','rail_buyer_quote_items.buyer_quote_id','=','rail_buyer_quotes.id')
			->where('rail_buyer_quote_items.id','=',$buyer_quote_id)
			->select('rail_buyer_quotes.from_location_id','rail_buyer_quotes.to_location_id')
			->get();
			...
			$row->cells [5]->value = ''; 
							$quoteid = DB::table('rail_buyer_quote_items')
							->where('rail_buyer_quote_items.id','=',$buyer_quote_id)
							->select('rail_buyer_quote_items.buyer_quote_id')
							->get();
							
							$quoteitems = DB::table('rail_buyer_quote_items')
							->join('lkp_load_types','lkp_load_types.id','=','rail_buyer_quote_items.lkp_load_type_id')
							->join('lkp_packaging_types','lkp_packaging_types.id','=','rail_buyer_quote_items.lkp_packaging_type_id')
							->where('rail_buyer_quote_items.buyer_quote_id','=',$quoteid[0]->buyer_quote_id)
							->select('rail_buyer_quote_items.*','lkp_load_types.load_type','lkp_packaging_types.packaging_type_name')
							->get();
							...
							<button class="btn red-btn pull-right submit-data underline_link seller_submit_quote" data-buyernbuyerquoteid="'.$buyer_id.'_'.$buyer_quote_id.'" id="click-link" >Submit Quote <i class="fa fa-rupee"></i></button>
							...
							<div class="col-md-12 padding-none">
								...
								<div class="col-md-3 padding-left-none form-control-fld padding-top">
									...
									placeholder="Rate per Kg *" class="form-control form-control1 fourdigitstwodecimals_deciVal numberVal " value="'.$getInitialQuotePrice[0]->initial_rate_per_kg.'" >
										'; 
									else
										$row->cells [5]->value .= '
								<div class="col-md-3 padding-left-none form-control-fld padding-top">
								<input type="text" name="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'" 
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'" 
                                    id="initial_quote_rateperkg_'.$buyer_id.'_'.$buyer_quote_id.'"
									placeholder="Rate per Kg *" class="ptl_initial_rate_per_kg fourdigitstwodecimals_deciVal numberVal form-control  form-control1" >';
								...
								<div class="col-md-3 padding-left-none form-control-fld padding-top">
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_initial_conversion form-control  fourdigitsthreedecimals_deciVal numberVal form-control1" >';
									$row->cells [5]->value .='</div>
									<div class="col-md-3 padding-left-none form-control-fld padding-top">
								...
								if(isset($getCounterQuotePrice[0]->counter_rate_per_kg) && $getCounterQuotePrice[0]->counter_rate_per_kg !='')
										$row->cells [5]->value .='
											...
											placeholder="Conversion Kg/CFT *" class="form-control form-group fourdigitsthreedecimals_deciVal numberVal margin-top"  value="'.$getCounterQuotePrice[0]->counter_rate_per_kg.'">
											'; 
									
								if(isset($getCounterQuotePrice[0]->counter_kg_per_cft) && $getCounterQuotePrice[0]->counter_kg_per_cft!=''){
									$row->cells [5]->value .='
								<div class="col-md-3 padding-left-none form-control-fld"><span class="data-head">Conversion KG / CFT </span> <span class="data-value"> '.$getCounterQuotePrice[0]->counter_kg_per_cft.' KG </span></div>
									<input type="hidden"
								...
								if(isset($getFinalQuotePrice[0]->final_kg_per_cft) && $getFinalQuotePrice[0]->final_kg_per_cft!='')
									$row->cells [5]->value .='
									<div class="col-md-3 padding-left-none form-control-fld margin-bottom-none"><span class="data-head">Conversion KG / CFT </span> <span class="data-value"> '.$getFinalQuotePrice[0]->final_kg_per_cft.' KG</span></div>';
								elseif(isset($getFinalQuotePrice[0]->final_kg_per_cft) && $getFinalQuotePrice[0]->final_kg_per_cft=='' && $getInitialQuotePrice[0]->initial_kg_per_cft!=''  &&  $getCounterQuotePrice[0]->counter_rate_per_kg!='')
                                    ...
                                    placeholder="Conversion Kg/CFT *" class="ptl_final_conversion fourdigitsthreedecimals_deciVal form-control form-control1 numberVal " ></div>
								...
								if(isset($getInitialQuotePrice[0]->initial_quote_price) && $getInitialQuotePrice[0]->initial_quote_price=='0.0000'){
                                    ...
                                    data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
                                    id="initial_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'"
                                    placeholder="ODA Rs *" class="ptl_initial_oda form-control fourdigitstwodecimals_deciVal form-control1 numberVal " value="0">';
									}
								}
								elseif(isset($getCounterQuotePrice[0]->counter_quote_price) && $getCounterQuotePrice[0]->counter_quote_price !='0.0000' && isset($getFinalQuotePrice[0]->final_quote_price) && $getFinalQuotePrice[0]->final_quote_price=='0.0000'){
									if($odacheck ==1){
	                                    ...
	                                    placeholder="ODA Rs *" class="ptl_final_oda form-control fourdigitstwodecimals_deciVal form-control1 numberVal ">';
									}else{
										$row->cells [5]->value .= '
										<input type="text"
			                            ...
			                            data-buyerid="'.$buyer_id.'" dat-buyerqouteit="'.$buyer_quote_id.'"
			                            id="final_quote_oda_'.$buyer_id.'_'.$buyer_quote_id.'"
	                                    placeholder="ODA Rs *" class="ptl_final_oda form-control fourdigitstwodecimals_deciVal form-control1 numberVal " value="0">';
									}
								}
								elseif(!isset($getInitialQuotePrice[0]->initial_quote_price)){
									if($odacheck ==1){
								...
								$row->cells [5]->value .= '</div></div><div class="col-md-4 data-fld padding-none text-right pull-right">
