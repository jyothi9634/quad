...
class RailQuotesComponent {
	 ...
	 * @return Response
	 */
	public static function RailSellerQuoteSubmit($request) {
		try{
			...
			if(isset($sellerInput['buyer_buyerquote_id']) && !empty($sellerInput['buyer_buyerquote_id'])) {
				...
				$getSellerpostdetails  = DB::table('rail_seller_post_items')
				->leftjoin('rail_seller_posts','rail_seller_posts.id','=','rail_seller_post_items.seller_post_id')
				->where('rail_seller_post_items.id','=',Session::get('seller_post_item'))
				->where('rail_seller_post_items.created_by','=',Auth::user()->id)
				->select('rail_seller_posts.*','rail_seller_post_items.*')
				->get();
				
			
				$getBuyerpostdetails  = DB::table('rail_buyer_quotes')
				->where('rail_buyer_quotes.id','=',$buyerQuoteItemId)
				->where('rail_buyer_quotes.created_by','=',$buyerId)
				->select('rail_buyer_quotes.*')
				->get();
				...
				$createsellerpost = new RailSellerPost();
				$createsellerpost->lkp_service_id = RAIL;
				...
				CommonComponent::auditLog($createsellerpost->id,'rail_seller_posts');
				...
				$createsellerpostitem = new RailSellerPostItem();
				...
				CommonComponent::auditLog($createsellerpostitem->id,'rail_seller_post_items');
				
				
				
				$sellerselectedbuyer = new RailSellerSellectedBuyer();
				...
				CommonComponent::auditLog($sellerselectedbuyer->id,'rail_seller_sellected_buyers');
				...
				//total amount
				$total_initail_amount = $totalfrieghtamount + $_REQUEST['pickupvalue'] + $_REQUEST['deliveryvalue'] +$_REQUEST['odavalue'];
					
				
				date_default_timezone_set("Asia/Kolkata");
                        //code added by swathi for updating values from market leads	
                        $getbqsqp = DB::table('rail_buyer_quote_sellers_quotes_prices as bqsp')
                        ...
                        if(count($getbqsqp)>0){
                                ...
                                DB::table('rail_buyer_quote_sellers_quotes_prices')
                                ->where('rail_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
                                ->where('rail_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$_REQUEST['cbuyerquoteid'])
				...
				$initial_cretaed = date ( 'Y-m-d H:i:s' );
				$createdIp = $_SERVER['REMOTE_ADDR'];
				$sellerinitialquote = new RailBuyerQuoteSellersQuotesPrice();
				...
				CommonComponent::auditLog($sellerinitialquote->id,'rail_buyer_quote_sellers_quotes_prices');
                        }		
	 ...
	 * @return Response
	 */
	public static function RailSellerFinalQuoteSubmit($request) {
		try{
			$formvalues = urldecode($_REQUEST['formvalues']);
			$formfields = explode("&", $formvalues);
			$hiddenfields = array();
			...
			CommonComponent::auditLog($buyerId,'rail_buyer_quote_sellers_quotes_prices');
	 ...
	 * @return Response
	 */
	public static function RailsellerSearchQuoteSubmit($request) {
		try{
			...
			$getBuyerpostdetails  = DB::table('rail_buyer_quote_items')
			->leftjoin('rail_buyer_quotes', 'rail_buyer_quotes.id', '=', 'rail_buyer_quote_items.buyer_quote_id')
			->where('rail_buyer_quote_items.id','=',$buyerQuoteItemId)
			->where('rail_buyer_quotes.created_by','=',$buyerId)
			->select('rail_buyer_quotes.*')
			->get();
			if(count($getBuyerpostdetails)>0){
		...
		//code added by swathi for updating values from market leads	
                        ...
                        if(count($getbqsqp)>0){
                                        ...
                                        $updateinitial = DB::table('rail_buyer_quote_sellers_quotes_prices')
                                        ->where('rail_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
                                        ->where('rail_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$_REQUEST['cbuyerquoteid'])
                                        ->where('rail_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
			...
			$initial_cretaed = date ( 'Y-m-d H:i:s' );
			$buyerinitial = new RailBuyerQuoteSellersQuotesPrice();
			...
			$buyerinitial->save();
			CommonComponent::auditLog($buyerinitial->id,'rail_buyer_quote_sellers_quotes_prices');
                }
			$seller_initial_quote_email = DB::table('users')->where('id', $buyerId)->get();
			$seller_initial_quote_email[0]->sellername = Auth::User()->username;
	 ...
	 * @return Response
	 */
	public static function RailsellerCounterAcceptance($request) {
		try{
			...
			$getcounter = DB::table('rail_buyer_quote_sellers_quotes_prices')
			->where('rail_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
			->where('rail_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$buyerQuoteItemId)
			//->where('ptl_buyer_quote_sellers_quotes_prices.seller_post_item_id','=',$sellerInput['seller_post_item_id'])
			->where('rail_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
			->select('rail_buyer_quote_sellers_quotes_prices.*')
				...
				->where('rail_buyer_quote_sellers_quotes_prices.buyer_id','=',$buyerId)
				->where('rail_buyer_quote_sellers_quotes_prices.buyer_quote_id','=',$buyerQuoteItemId)
				->where('rail_buyer_quote_sellers_quotes_prices.seller_post_item_id','=',$sellerInput['seller_post_item_id'])
				->where('rail_buyer_quote_sellers_quotes_prices.seller_id','=',Auth::user()->id)
				->select('rail_buyer_quote_sellers_quotes_prices.*')
				->get();
			}
	 ...
	 * @return Response
	 */
	public static function PTLsellerQuoteAcceptance($id,$bqid,$spqi=null) {
		try{
